...
- **Klimazonen** (Entwurf `docs/superpowers/specs/2026-08-03-klimazonen-design.md`, Bauplan `docs/superpowers/plans/2026-08-03-klimazonen.md`) — die **vierte** Landschaften-Ebene (`kind='klima'`): sieben Zonen von Nord nach Süd, Polar bis Tropisch. 💣 **Die sechs Trennlinien (`ecosystem_climate_divider`, LineString) sind die Wahrheit; die sieben Bänder sind ABGELEITET** und werden bei jedem Speichern neu gerechnet (`avesmapsEcosystemClimateRebuildBands`). Dadurch ist „keine Überlappung" **Bauart statt Regel** — ein Band *ist* der Raum zwischen zwei Linien —, und weil die Bänder gewöhnliche `ecosystem_area`-Zeilen sind, erben Regionen-Editor, „Zugehörigkeit rechnen" und die Wege-Zuordnung sie ohne eigenen Code. 💣 **Ein Band darf NIE als Polygon bearbeitet werden**: `avesmapsClimateAssertNotDerived()` verweigert create/update/delete/terrain serverseitig, und `update_region` lässt Name und Wiki durch, aber nicht Ebene oder Art (die Art sagt, *welche* Zone das ist). 💣 **Auf jeder Trennlinie muss `x` streng steigen** — nur dann ist sie eine Funktion `y(x)`, nur dann prüft der Reihenfolge-Wächter exakt (an der Vereinigung beider x-Mengen, kein Raster), und nur dann ergeben zwei Linien ein Polygon ohne Selbstschnitt. ⚠️ Die Saat läuft im SCHREIB-Dispatcher (`climate_get`), **nie in `EnsureTables`** — die hebt die Revision nicht, und der Bestand käme als 304 an. ⚠️ Die Zonen tragen **kein `map_features`-Label**: ihr Name wird von der Ebene gezeichnet und ist deshalb als einziger `type_key` vom Label-Subtyp-Zwang ausgenommen. Kein Reise-Effekt in dieser Fassung — das ist eine eigene Sitzung.
