...
- **Das Fenster „Neuigkeiten"** (Mockup der Abstimmung: `docs/changelog-mockup.html`, live 2026-08-03) — die Meilensteine des Projekts, erzählt für Leser, die weder Commits noch Code kennen. 🔴 **Bis zum 09.08.2026 hieß es „Änderungsverlauf"** — umbenannt wurde ausschließlich die BESCHRIFTUNG (Fenstertitel, Kachelknopf, aria-Label, Fehlersatz und die deutschen Gegenstücke in `i18n-en.js`). Dateiname, CSS-Klassen, ids, die **i18n-Schlüssel selbst** (`changelog.*`), der Endpunkt und die Tabelle `changelog_entry` heißen weiter changelog — dieselbe Trennung wie beim Literatur-Umbau und aus demselben Grund: der Deploy löscht nie (§10), eine umgetaufte Adresse liesse eine noch gecachte `index.html` ins Leere greifen. Erreichbar über die **Hinweise → Karte „Was ist neu?"**, direkt unter der Discord-Karte, als deren Zwilling, **und seit 09.08.2026 direkt über den Eckknopf „Neuigkeiten"** links neben „Hinweise" unten rechts an der Karte. 💣 **Die Ecke ist seither EIN Bund** (`#map-corner-actions`): das Positionieren gehört dem Behälter, nicht mehr `#legal-button` — sonst wanderte nur einer der beiden mit, wenn die Infobox aufgeht. Wird es eng (offene Infobox auf schmalem Schirm), stapelt der Bund: „Neuigkeiten" nach oben, „Hinweise" bleibt unten am Rand. Wie hoch der Bund baut, sagt **eine** Zahl, `--avesmaps-corner-stack`; der Leaflet-Zoom darüber rechnet seinen Abstand daraus, statt wie früher eine abgeschriebene `bottom: 52px` zu tragen. ⚠️ Die Zahl hängt an `:root`, **nicht** an `.avesmaps-infopanel-mode` allein — die Klasse sitzt an `<html>` UND `<body>`, und die body-Kopie überschrieb die Ausnahme für den schmalen Fall, sodass der Zoom auf der oberen Knopfreihe saß (so gemessen, dann behoben). Das Fenster hat seither zwei Öffner; der Fokus kehrt zu dem zurück, der es aufgemacht hat. Bewacht von `js/app/__tests__/map-corner-actions.test.js`. Fenster `#changelog-overlay` (`js/app/changelog-dialog.js`, `css/components/changelog-dialog.css`), Lesepfad `GET /api/app/changelog.php`, Schreibpfad `POST /api/edit/map/changelog.php` (`list`/`save`/`delete`, capability `edit`), Logik + Startbestand `api/_internal/app/changelog.php`. 💣 **Zwei Quellen, eine Rangfolge:** die Tabelle `changelog_entry` ist die Wahrheit, sobald sie steht — solange sie fehlt ODER leer ist, antwortet der Lesepfad aus der Konstante `avesmapsChangelogSeed()` (42 Meilensteine). Das ist Absicht: der Verlauf steht sofort nach dem Deploy, und der Lesepfad braucht **kein DDL** (das läuft sonst bei jedem Besucher, der die Hinweise öffnet, und macht die Datei ohne lebende Datenbank untestbar). Angelegt und geseedet wird ausschliesslich im Schreibpfad, und `avesmapsChangelogSeedIfEmpty()` füllt **nur eine leere Tabelle** — ein von Hand nachgeschärfter Eintrag wird nie von der Konstante überfahren. 💣 **Die Einträge sind Fliesstext für Leser, also mit echten Umlauten** — die ae/oe/ue-Umschreibung des Hausstils gilt für Kommentare, nicht für das, was im Fenster steht; `changelog-test.php` bewacht genau das (die Saat war beim Bau schon einmal umgeschrieben). 💣 **`overflow-anchor: none` am Scroll-Kasten ist tragend:** ohne es steht der frisch geöffnete Verlauf beim ÄLTESTEN Eintrag, weil das Scroll-Anchoring den Kasten mitzieht, wenn „Lade …" durch 42 Einträge ersetzt wird. Übersetzt wird nur der Rahmen (`changelog.*` in `i18n-en.js`); die Einträge selbst bleiben deutsch — Projektgeschichte ist Inhalt, keine Oberfläche. Gepflegt wird der Verlauf von der Routine **„Avesmaps feature updates"**: `list` liefert ihr `latest_source_ref` (den Commit, bis zu dem er reicht), `save` hängt an. 💣 **Diese Routine hat keine Session, sie hat nur ihr App-Token** — und bis 2026-08-08 verlangte der Schreibpfad ausschliesslich eine Session. Damit konnte ihn NIEMAND rufen: es gibt ausser der Routine keinen Aufrufer (keine Oberfläche verlinkt ihn, `git grep "edit/map/changelog"` über `js/` und `edit/` ist leer), und so stand der Verlauf fünf Tage nach seinem Start noch unangetastet auf der Saat — `source: "seed"`, 42 Einträge, jüngster 03.08. Das fiel nicht auf, weil ein Verlauf aus der Konstante genau wie ein gepflegter aussieht. Seither nimmt der Endpunkt **zwei Ausweise**: Session mit capability `edit` **oder** ein Token im Header `X-Avesmaps-Token`. 🔴 **Das Token hat einen EIGENEN Schlüssel — `$config['changelog']['app_token']` in `api/config.local.php`, nicht den von Discord.** Die erste Fassung lieh sich `discord.app_token`, weil das keine neue Konfigurationszeile kostete; der Owner hat das noch am selben Tag umgedreht, und zu Recht: damit hätte ein Token, das bis dahin nur in einen Chat-Kanal schreiben durfte, öffentlich sichtbaren Text in die Webanwendung geschrieben. Zwei Befugnisse an einem Schlüssel lassen sich weder einzeln tauschen noch einzeln sperren, und ein Leck auf der einen Seite öffnet die andere mit. Deshalb hat der Verlauf auch seine **eigene** Prüfung (`avesmapsChangelogTokenMatches()`, `hash_equals` + beidseitiger Leer-Riegel) statt `avesmapsDiscordCheckAppToken()` — drei Zeilen doppelt sind der Preis dafür, dass die beiden Türen nichts voneinander wissen. ⚠️ **Fehlt der Schlüssel, fällt der Weg ZU, nicht auf** — dann bleibt nur die Session. ⚠️ Das Token darf **weniger** als ein Mensch: nur `list` und `save` (`AVESMAPS_CHANGELOG_TOKEN_ACTIONS` + `avesmapsChangelogTokenMayRun()`), niemals `delete` — ein abhandengekommenes Token soll ergänzen können, nicht ausräumen, und die Routine hat zum Löschen keinen Grund. Gelesen wird es **nur aus dem Header**, nie aus `?token=` wie in `report-post.php`: eine Adresszeile steht im Server-Log. Bewacht von `api/_internal/app/__tests__/changelog-token-gate-test.php` (sechs Mutationen rot, darunter der totgelegte Riegel `if (false && …)` und der Rückfall auf den Discord-Schlüssel). 💣 **Falle beim Schreiben solcher Quelltext-Zusicherungen:** in dieser Datei haben nacheinander **drei** davon zuerst auf den erklärenden **Kommentar** angeschlagen statt auf den Code (`AVESMAPS_CHANGELOG_TOKEN_ACTIONS`, `discord`, `hash_equals`) — beim Quelltext-Test ist die Prosa Teil des Suchraums. Nie auf einen Bezeichner prüfen, immer auf den Aufruf samt Argumenten.
...
- `docs/abenteuer-feature-design.md` (spec) + `docs/abenteuer-instruction.md` (phased build) + `docs/abenteuer-editor-ui-spec.md` (Phase 3 / P2 editor-UI spec — layout C, multiple starts, manual ▲▼ reorder, hidden suppressed-tombstones toggle, token-based) — **Literatur feature** (display name „Literatur"; until 2026-08-07 the whole feature was called „Abenteuer"): DSA game literature ↔ its ordered places (settlement/territory/region/path) surfaced as infopanel sections + a spoiler-gated questroute. **Phase 1 shipped**: schema `adventure`/`adventure_place` (inline DDL), catalog read `GET /api/app/game-literature.php` + wiki-key resolver + client index, settlement "beginnt/spielt (Spoiler)" display. **Phase 2 (aggregation) + Phase 3 editor P1 (backend) + P2 (UI, layout C) + P3 (Ort-autocomplete, `docs/abenteuer-editor-p3-autocomplete.md`) shipped**; regions resolve via nested `properties.wiki_region.wiki_url` (a top-level `wiki_url` is empty for regions); a picked non-wiki-linked place shows "ohne Wiki-Eintrag" (a valid state). **Phase 4 wiki-sync (dump → adventures) + „Literatur"-Ribbon + Cover shipped** (mirrors the publication two-step: an `adventures` staging phase in the dump-hybrid driver builds `wiki_adventure_catalog`/`wiki_adventure_place_staging` from `{{Infobox Produkt}}` adventure pages during „Dump holen"; an owner-triggered `sync_adventures` action reconciles override-safely into `adventure`/`adventure_place` — writes/deletes ONLY `origin='wiki'`, manual/community/suppressed untouched, adopts the 6 seed placeholders by title. Real infobox fields: `Ort`/`Art`/`Genre`/`KompM`/`KompSp`/`Regeln`/`Bild={{ProdCover}}` — no BF year in the infobox. **Covers** ride the Wappen engine: fetched to `/uploads/questcovers` on reconcile, own>wiki override, F-Shop-reference license (NOT `public_domain`); the frontend cover links to the Ulisses F-Shop + a source footnote, the editor has a cover preview + upload/„neu ziehen". Reconcile/parser/driver cores are unit-tested; end-to-end needs a real „Dump holen" + „Literatur syncen"). Then editor-P4 community — its own session.
  💣 **Die DRITTE Rolle `covers` („beschreibt"), live 2026-08-07** (Entwurf `docs/superpowers/specs/2026-08-06-literatur-design.md` §§4–7). Ein Ort trägt seit dem Literatur-Umbau eine von **drei** Rollen: `start` („beginnt hier", spoilerfrei) · `play` („spielt hier", **play IST der Spoiler**) · `covers` („beschreibt", spoilerfrei — Regionalspielhilfe und Spielhilfe, deren Orte aus `Thema` kommen). 💣 **Jede Stelle, an der „alles, was nicht start ist, ist Spoiler" stand, war damit falsch** — und zwar lautlos: eine Regionalspielhilfe stünde hinter einem Schleier „Spoiler (spielt hier)" über einem Buch, das nichts verrät. Die Regel steht deshalb an **einer** Stelle je Seite und wird von dort aus benutzt: `avesmapsGameLiteratureNormalizeRole` in `js/map-features/map-features-game-literature.js` (Client) und in `api/_internal/app/game-literature.php` (Server). 💣 **Der Rückfall geht IMMER Richtung Spoiler:** Unbekanntes wird `play`, nie `start`/`covers` — ein zu Unrecht verschleierter Eintrag ist ein Schönheitsfehler, ein zu Unrecht offener verrät, wo ein Abenteuer hinführt. ⚠️ **Visuell unterschieden wird über die ART auf der Karte** (`.avesmaps-adv__type`: „Regionalspielhilfe"), NICHT über eine zweite Farbfamilie oder einen neuen Marker (Entwurf §7 + §12). `.is-play` heißt am Element weiterhin **nur** „verschleiert"; die volle Rolle steht in `data-role`, und der flache Dialog baut seine Zeilen aus **diesem** Attribut — aus der Klasse gelesen käme jede „beschreibt"-Karte als Startort zurück. ⚠️ **Der Kopfzähler „Literatur zu X (N)" zählt, was OFFEN dasteht** (`start` + `covers`), nicht den Streifen: ein Spoiler steht sichtbar da, zählt aber nicht mit, sonst verriete die Zahl allein schon, wie viel hier gespielt wird. ⚠️ Der Hinweis „Hier beginnt kein Abenteuer." erscheint nur, wenn **gar nichts** offen dasteht — über einer offenen Karte wäre er ein Widerspruch. ⚠️ **Die Spotlight-Suche liest `role IN ('start','covers')`** (`api/_internal/app/game-literature-search.php`): `covers` musste dazu, weil eine Regionalspielhilfe überhaupt keinen Startort hat und sonst ohne Sprungziel ankäme; `play` bleibt draußen, und ein Test hält fest, dass in dieser Datei **kein SQL-Literal** dafür vorkommt — auch nicht im Kommentar. Der Client wortet daraus „beginnt in {Ort}" bzw. „beschreibt {Ort}" (`place_role` im Payload). ⚠️ **Im Editor entscheidet die gespeicherte Rolle der ZEILE, die Art des Werks nur den Vorgabewert** für neu zugeordnete Orte (`aeNewPlaceRole`) — andersherum machte ein Umstellen des Produkttyps aus 40 Spielorten stillschweigend 40 beschriebene. Eine `covers`-Zeile zeigt statt des ★-Umschalters eine Pille „beschreibt". Ebenso `avesmapsGameLiteratureLinkPlaceFromSource`: die Rolle kommt dort aus `avesmapsGameLiteratureRoleForKind`, nicht mehr als Literal `'play'` — genau dieser Weg (Quelle eintragen → Ort verknüpft) ist für eine Regionalspielhilfe der wahrscheinlichste von allen. 💣 **`PRODUCT_TYPE_GROUPS` im Editor ist der Zwilling von `AVESMAPS_GAME_LITERATURE_KINDS`** (`api/_internal/wiki/publication-parsing.php`) und trägt dieselbe `roles`-Spalte; `PRODUCT_TYPES` ist daraus **abgeleitet**, nie zweitgeschrieben. `publication-parsing-test.php` liest den Editor-Quelltext und hält beide Listen samt Rollen-Spalte gegeneinander — das ist Falle §4b (2026-07-19: `kampagne` gegen „Kampagnenband", 27 Bände) als Zusicherung. ⚠️ Ein **unbekannter** gespeicherter `product_type` bekommt im Auswahlfeld eine eigene Gruppe „nicht zugeordnet", statt still auf die erste Option zu rutschen — die Weiche lässt jedes `Art` durch, in dem „abenteuer" oder „kampagne" steckt, der Editor kennt aber nur die ausgeschriebene Liste.

  💣 **Renamed 2026-08-07 (Abenteuer → Literatur).** Files are `game-literature*`, identifiers `avesmapsGameLiterature*` / `gameLiterature*` / `GAME_LITERATURE_*`, and every surface says „Literatur" — except where the word names the **Art** (the product type „Abenteuer", the roles „beginnt hier"/„spielt hier", the source-type option), which stays. 🔴 **The TABLES kept their names** (`adventure`, `adventure_place`, `adventure_link`, `wiki_adventure_*`) and so did every string that stands as a VALUE in a row: the `app_setting` keys (`adventures_enabled`, `adventure_covers_enabled`, `adventure_cover_autoget_enabled`, `adventures_last_synced`), the POST/lock action `sync_adventures`, the dump cursor `adventure_cursor`, the audit actions `delete_adventure`/`upsert_adventure` and their `adventure_public_id` key, the sync-plan/decision `kind` `'adventure'`, the conflict `type`/`claim_source`, the linkcheck entity key, and the JSON payload keys (`location.adventures`, `adventure_total`, `adventures_done`). That split is what makes the rename safe: the deploy never deletes (§10), so `api/app/adventures.php` and its whole old require chain stay on the server as orphans — and because the tables are unchanged, **those orphans keep answering correctly** for a visitor with a cached `index.html`. A `RENAME TABLE` would break exactly that. ⚠️ The CSS classes `.avesmaps-adv*` also stayed: the Kartensammlung dialog and the generic filter bar (`js/ui/filter-bar.js`) share them, so they name a COMPONENT, not this feature. Spec: `docs/superpowers/specs/2026-08-06-literatur-design.md` §8.
- **Die Übernahme-Vorschau** (Entwurf `docs/superpowers/specs/2026-08-06-sync-uebernahme-design.md`, Bauplaene `docs/superpowers/plans/2026-08-06-sync-uebernahme-sitzung-{1,2}.md`, Mockup `docs/sync-uebernahme-mockup.html`) — **jeder Wiki-Abgleich zeigt erst, was er tun würde; geschrieben wird nur, was angehäkelt ist.** Drei Kategorien (**Neu · Geändert · Gelöscht**), Neu und Geändert vorangehäkelt, Gelöscht nie und mit zweiter, ausdrücklicher Bestätigung. 💣 **Sie kommt IMMER — auch wenn nichts anders ist** (Owner-Entscheid 2026-08-07, davor blieb sie bei null Unterschieden zu). Wer einen Abgleich anstößt, fragt nicht „gibt es Arbeit", sondern „hat sich etwas getan", und ein Fenster, das mal kommt und mal nicht, beantwortet weder das eine noch das andere. Die alte Begründung („ein leeres Blatt nach zehn Minuten Arbeit ist Lärm") stimmte nur, solange das leere Blatt tatsächlich stumm war: drei Gruppen mit einer Null. Deshalb sagt es den Befund jetzt in **einem Satz** und mit **allen drei Zahlen, Nullen eingeschlossen** (`syncPlanVerdict`) — „0 gelöscht" unterscheidet „es wird nichts gelöscht" von „danach wurde gar nicht gesucht" —, dazu den Stand des Dumps, gegen den verglichen wurde: „keine Unterschiede" ist keine Aussage über das Wiki, sondern über den zuletzt geholten Dump. Und es hat **keinen Übernehmen-Knopf**, wo es nichts zu übernehmen gibt; sein einer Knopf heißt dann „Schließen", nicht „Später". ⚠️ Erlaubt bleibt der Riegel `run_id > 0`: keine Lauf-Nummer heißt, dass die Rechen-Hälfte nicht fertig wurde. Bewacht von `js/review/__tests__/sync-plan-always-opens.test.js`, das alle fünf Aufrufstellen abgeht — die Regel fällt sonst beim nächsten „ich mach das hier mal wieder zu" lautlos zurück, weil es wie eine Aufräumung aussieht. ⚠️ Der Weg zurück in eine mit „Später" liegengelassene Liste ist eine Statuszeile im jeweiligen Editor (`refreshCePlanHint` / `refreshAePlanHint`, Vorbild `refreshPlanStatus` im Territorien-Monitor) über `syncPlanOpenRunSummary` — **nie ein zweiter `fetch` auf `/api/edit/wiki/sync-plan.php`**, die Adresse steht an genau einer Stelle und ein Test zählt die Nennungen. Drei Tabellen: `sync_plan_run` / `sync_plan_item` (bei jedem Lauf neu gerechnet) und 🔴 `sync_decision` — **die EINZIGE dauerhafte Entscheidung**, nie automatisch geleert, rücknehmbar über „früher abgelehnte Löschungen anzeigen". Bauteil `js/review/sync-plan-sheet.js` + `css/components/sync-plan-sheet.css`, Endpunkt `api/edit/wiki/sync-plan.php` (`get`/`select`/`apply`/`declined`/`undecline`, Fähigkeit `edit`), Fundament `api/_internal/wiki/sync-plan.php`. **Live seit 2026-08-06 für Stadtkarten, Abenteuer, Publikationsquellen und Vorkommen** (Sitzungen 1+2); Territorien (4) ziehen nach. 💣 **Sitzung 3 (Orte, Wege, Regionen) hat bewusst KEINE `sync_plan_item`-Zeilen bekommen** — dort bestätigt schon heute ein Mensch jeden Fall einzeln, und die drei Kategorien passen nicht: von 16 Falltypen sind 2 ehrlich „Neu“, 7 „Geändert“, **0** „Gelöscht“ und 7 gar keine Änderung, sondern ein Problem („zwei Orte heißen gleich“). Ein Häkchen ist bei **2 von 16** die richtige Form; sonst ist die Antwort „welcher von diesen“ und danach ein Formular. Diese drei Abgleiche bekamen deshalb nur die **Formensprache** des Blattes (`.wiki-sync-case*` in `css/features/review-panel.css`), ihre Verben und ihre eigene Speicherung (`wiki_sync_cases`) bleiben — gemeinsame Oberfläche, eigene Speicherung, genau wie im Konfliktzentrum. Zählung und Begründung: Entwurf §7, Mockup `docs/sync-uebernahme-fallliste-mockup.html`. ⚠️ Erfasst sind **Orte, Wege und das Konfliktzentrum** — sie teilen sich `.wiki-sync-case*` und erbten die Form in einem Zug (die Wege ohne eine Zeile JS). Die **Regionen NICHT**: ihre Liste schreibt eigene Klassen (`region-sync__*`) und ist keine Fallliste, sondern eine Arbeitsfläche zum Draufziehen. ⚠️ Wer es doch versucht, baut den am 22.07. gemessen ausgebauten Wege-Sammelknopf wieder ein, diesmal vorangehäkelt. Das Blatt selbst ist seit Sitzung 3 für eine **zweite Zeilenquelle** offen (`syncPlanResolvePost`, `openSyncPlanSheet({…, post})`) — das ist der Andockpunkt für Sitzung 4. Seither endet auch „Dump holen" in einer Vorschau (Schritt 4/4 ist der Quellen-Abgleich). 💣 **Die dritte Kategorie gehört dem Verschwinden einer ganzen EINHEIT.** Verliert eine lebende Einheit nur Kindzeilen, die das Wiki nicht mehr auflistet (3 Orte eines Abenteuers, 2 Quellenverweise eines Ortes, 4 Vorkommen eines Eintrags), steht das benannt und in Warnfarbe in ihrer **Geändert**-Zeile — nicht unter „Gelöscht". Der Grund ist zwingend: die Ausführ-Hälfte ruft den unveränderten Schreiber, und der schreibt eine Einheit GANZ; zwei Zeilen je Einheit hiessen, dass eine abgehäkelte Löschung trotzdem passiert. Abenteuer und Publikationsquellen haben deshalb NIE eine Löschzeile, und ihre dritte Gruppe sagt das ausdrücklich. 💣 **Bei den Vorkommen ist die „Löschung" ein GRABSTEIN:** `status='retired'`, der Eintrag behält Vorkommen, Quellen und `wiki_key`, und der nächste Abgleich weckt ihn selbst wieder. Bestätigung, Knopf und Protokoll sagen deshalb „stilllegen", nicht „löschen" (`AVESMAPS_COLLECTION_AUDIT_KIND_DELETION_VERB`) — eine Warnung, die mehr behauptet als passiert, wird weggeklickt, und danach auch die, die stimmt. 💣 **Jeder Abgleich ist in zwei Hälften geschnitten:** die Rechen-Hälfte (`avesmaps{Citymap,Adventure,Publication,Lore}PlanStep`) schreibt in KEINE Nutztabelle, die Ausführ-Hälfte (`api/_internal/wiki/*-plan-apply.php`) ruft den **unveränderten** Schreiber. 💣 **Wo eine Vorlage durch SCHREIBEN antwortet, braucht die Rechen-Hälfte einen read-only Zwilling:** `avesmapsAdventureFindOrAdoptRow` übernimmt einen Platzhalter auf der Stelle (→ `avesmapsAdventurePlanFindRow`), `avesmapsFeatureSourceUpsert` legt die Quelle an, um ihre id zu nennen (→ `avesmapsPublicationSourceIdForPlan`), und das Titelbild eines Abenteuers wird mitten im Schreiben über HTTP geholt (die Zeile sagt „wird neu geladen", geladen wird beim Übernehmen). Bewacht von `__tests__/sync-plan-purity-test.php`, das den ganzen Aufrufbaum abläuft — samt Gegenprobe, dass derselbe Lauf von der Ausführ-Hälfte aus die Schreiber FINDET. 💣 **„Löschung abgelehnt" ist NICHT `origin='manual'`.** Das entschiede zwei Dinge auf einmal — „nicht löschen" *und* „nie wieder aktualisieren" —, und in dem Moment denkt man nur über das erste nach: die Zeile bleibt `origin='wiki'`, behält ihren `wiki_key` und läuft weiter mit; kommt der Artikel zurück, ist sie ohne Zutun wieder dabei. 💣 **Eine Übernahme schreibt EINE Protokollzeile je Lauf** (`apply_sync_plan`), nie eine je Eintrag — `map_audit_log` behält 200 Zeilen, 46 Löschungen einzeln zu protokollieren räumte das Protokoll mit einem Klick leer. ⚠️ Der Leerkatalog-Riegel (`avesmapsCitymapRemovableKeys`) wandert unverändert mit: leerer Katalog ⇒ null Löschzeilen, sonst schlüge eine misslungene „Dump holen" 457 Löschungen vor. ⚠️ Der Löschriegel steht **serverseitig** in `apply` (`delete_not_confirmed`), nicht nur am ausgegrauten Knopf. ⚠️ Der Typ-Riegel der Quellen (`avesmapsPublicationStagingHasEntityType`) wird in BEIDEN Hälften gefragt: zwischen Vorschau und Übernahme kann ein neues „Dump holen" das Staging leeren, und „keine Wunschliste" heisst dann wieder „ich weiss nichts" — der Beinah-Unfall vom 22.07. (~34.800 Lore-Verknüpfungen). ⚠️ Die require-Kette des Vorschau-Endpunkts ist eine EIGENE (er ist nicht `dump.php`) und wird von `__tests__/sync-plan-endpoint-chain-test.php` geprüft: sie fand am 06.08. auf Anhieb, dass `avesmapsWikiSyncNextMapRevision` fehlte — die Karten-Übernahme hob die Kartenrevision nicht, lautlos hinter einem `function_exists`.
