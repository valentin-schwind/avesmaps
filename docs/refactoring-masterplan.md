...
| **M2** | Correctness bugs | next | route-graph shadow, loader trio (TOCTOU / cache invalidation / `apiUnavailable` logic), `askRegionTabCloseChoice`, `refreshPlannerAfterFeatureChange` dup, IZ/`bis` parsers, spotlight poll cancel, zoom-band table unification (⚠️ editor-behaviour — per-item sign-off) |
