...
| **M2** | Correctness bugs | in progress | ✅ done: IZ/`bis` word-boundary parsers (`9d4a801b`), spotlight poll cancel (`70a8c1e9`), zoom-band unified to 0-1/2-6 (`8fa18991`). ⏳ remaining (runtime-sensitive, do with live smoke test): route-graph `calculateRouteServer` shadow removal, political loader trio (TOCTOU / fan-out cache invalidation / `apiUnavailable` inversion / pending-rerun), `askRegionTabCloseChoice` 3-way dialog (owner chose save/discard/cancel). ↪ moved to M5: `refreshPlannerAfterFeatureChange` dedup (harmless dead code in a CRLF file — clean up during the routing split). |
