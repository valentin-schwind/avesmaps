...
| **M5** | God-file splits | ✅ done (substantively) | safe-split pattern (no-build classic scripts): extract a contiguous run of **global function declarations** (no top-level state, no load-time code) into a sibling file loaded adjacent in `index.html` → behaviour-identical (functions are global + hoisted, called at runtime). ✅ `review-wiki-sync.js` **1795→692** across two splits (`85ac848f`+`7a76f8bf`): case-list (grouping/filtering/accordion/rendering/create-location) → `review-wiki-sync-cases.js` 725; case-resolution (action buttons/focus-preview/location-pick/resolve dialog) → `review-wiki-sync-resolve.js` 390; all live-verified stamped + served. NOTE: `territory-wiki-tree.js` 947 is already a clean encapsulated IIFE module (zero top-level fns) — **not** a god-file-split candidate; leave as-is. ✅ `map-features.js` **1506→474** (`52a66803`+`81390da9`+`a95f6d39`, cluster complete): region split/move editing → `map-features-region-edit-ops.js` (252), location/crossing marker editing → `map-features-location-editing.js` (381), region geometry CRUD → `map-features-region-crud.js` (403); flat-globals safe pattern, shared state referenced cross-script; all live-verified stamped+200. The map-features/ cluster is now 52 modules, largest ~744, core thin. `embedded.js` is a single **IIFE closure module** (134 closure-scoped fns, 36 shared state vars) — the trivial function-move does NOT apply. Investigation (4 correspondence subagents) found the four suspected "dup engines" are mostly NOT clean dups: color-utils uses fraction-HSV vs embedded percent-HSV (delegation would corrupt colors); display-state is a different abstraction (no name overlap); inheritance is a different mechanism (after-save vs immediate) AND the module already runtime-shadows embedded's 2 inherit handlers. **The real win = dead-code removal:** embedded already delegates tree build/normalize/render to `territory-wiki-tree.js` at runtime (conditional override block). ✅ **Stufe 1 done (`f5923461`): removed the dead tree-building engine** (normalizeApiRows+buildTerritoryTree bodies → fallback stubs; 22 helpers + registerTree/sortTree/compareTreeNodes deleted; reachability-proven dead-only; createTreeNode kept). embedded.js **3555→3059**, ASSET_VERSION→20260614a. ⏳ needs an editor browser smoke (curl can't exercise the editor UI). Remaining optional: 2 runtime-dead inherit handlers (~75 lines, inert/harmless — leave unless wanted), local render chain (dead but interleaved with live `renderDropZone`). ⏳ remaining tiers: ✅ `review-panels.js` **822→471** (`06753785`): change-log/audit feed (load+render history, focus audited feature, undo + shortcut) → `review-panels-change-log.js` (flat-globals safe pattern, no top-level state); live-verified stamped+200. ✅ `spotlight-search.js` **910→673** (`a9622f62`): focus/navigation (bounds, pan/zoom to selected location/label/region/path, highlight, region-infobox poll, clear) → `spotlight-search-focus.js`; shared `let` state referenced cross-script; live-verified. (a) more `map-features.js` clusters; (c) `embedded.js` delegation refactor (separate, owner sign-off); (d) PHP giants — ✅ `sync-monitor.php` **3854→758** (−80%, `37092719`→`d63a5d65`): extract cohesive function clusters into sibling libs `require_once`'d after the consts so all 4 callers (sync-monitor/paths/settlements/regions endpoints) get them transitively — `-licenses.php` (312), `-parsing.php` (553), `-identity.php` (839), `-model.php` (model derivation + hierarchy editing, 920), `-tree.php` (tree/audit/wiki-rows/clear/sample, 503); core is now consts + crawl/BFS engine. Function-only libs, const/core deps resolve at call time; all 4 endpoints live-verified 401 (no 500) after each. ✅ `territories.php` **2529→951** (`457279af`+`a0797aec`): political-tree building → `territories-tree.php` (621), wiki HTML/wikitext parsing → `territories-parsing.php` (977); core is now cache/refresh/fetch-rows. ⚠️ that file had a copy ending with `}` and NO trailing newline at one point — `split` keeps `}` as the last element, so always cross-check `wc -l` vs `len(split())`. ✅ `locations.php` **1626→1044** (`fd8de3a5`): support helpers + map-feature plumbing + restored sync helpers → `locations-helpers.php` (582); lib pattern, 5 callers transitive (incl. settlement-coat-upload), all live-verified 401. ✅ `edit/map/features.php` endpoint **2309→75** (`192bae22`): all 97 handler functions → new lib `api/_internal/map/features.php` (`require_once`'d before the try/dispatch); endpoint keeps consts + `AvesmapsConflictException` + try/catch match-dispatch. Live-verified POST→401, OPTIONS→204, GET→405, lib→403 (htaccess-denied). **All four PHP giants now done.** ⏳ optionally more clusters from the now-mid-size libs; (e) CSS source split, rename duplicate `political-territory-editor.css`, treat `*-inline.css` as generated. ✅ asset tidy-up: flattened `icons/small_webp/` → `icons/` (40 webp, `44f30cc7`) + rewired index.html/config.js/css/generator + retired old server copies (`fcc39f0d`, manual full deploy — a workflow-only push self-skips via `SKIP_DEPLOY`); live-verified new 200 / old 404. ✅ **FINAL splits:** `derived-geometry-editor.js` 934→688 (geometry math → `territory-derived-geometry-math.js`, `f1ce9eff`); `territory-editor-link.js` 764→585 (save/assignment → `territory-editor-link-save.js`, `a7735764`); `review-settlement-list.js` 721→381 (bulk ops → `review-settlement-list-bulk-ops.js`, `637e6e23`). ✅ **CSS:** duplicate basename disambiguated — `css/components/political-territory-editor.css` → `…-overlay.css` + the one `@import` updated (`ebcab37f`); `*-inline.css` confirmed generated (auto-gen `tools/scope_editor_css.js`, kept tracked — no build step). **DEFERRED (reasons):** `routing.js` 1272 = planner bootstrap (load-time interleaved mid-file → not a clean split target); `embedded.js` `ctx`-delegation refactor (owner sign-off + contested WIP must settle). **→ M5 god-file splitting comprehensively done.** |
| **M6** | Performance | ✅ substantively done | ✅ **derived-layer N+1 eliminated (`1e4d5bc4`, the AGENTS.md §10 hotspot):** the rebuild looped over each derived feature calling 2 helpers that each ran a full `political_territory` scan (~2×D scans); now the immutable snapshot is fetched ONCE before the loop and threaded in (write paths still fetch fresh via the optional param). Behaviour-verified: Jilaskan derived-source ids identical pre/post, layer 200/699 features, POST→401. ✅ **layer/model_tree cache coherence** (`9e6629f9`+`ab5d8723`, owner-confirmed): green tree status + immediate flush on every editor write. Subagent analysis found polylabel is already memoized at write (no defect) and the `SHOW COLUMNS` capability probes are already statically cached. ⏳ optional lower-value/higher-care (deferred): batch the 2 per-feature `IN(...)` lookups into one each (1c), batch the contested `FetchClaimPartiesByPublicId`, frontend political teardown→signature-skip on same-zoom pan. ✅ coat-of-arms blink — region-label layer reuse instead of teardown (`adeb4f3e`, pulled forward; see below). ✅ DDL out of cache-hit path (`1679f644`): a fresh political-layer cache hit now serves the prebuilt JSON before `avesmapsCreatePdo` / ensure-tables DDL run — the AGENTS.md §10 hottest endpoint; live-verified `X-Avesmaps-Layer-Cache: miss`→`hit`, hierarchy still 200, POST→401. ⏳ remaining: derived-layer N+1 memo, political teardown → signature-skip (full reuse, not just labels), polylabel memo, fetch-interceptor, bound+invalidate fan-out cache |
| **M7** | Schema / SQL | ✅ baseline done (`3620badc`) | ✅ `sql/schema.sql` baseline created (824 lines, 29 live + 2 dead tables; `-- source:` per table, PHP-constant names resolved to literals, runtime ALTER-cols folded in, `LIKE` preserved, `territory_id` NULL = PHP-authoritative). Inline DDL stays the runtime source of truth; the file is reference + fresh-DB bootstrap (not loaded). FINDINGS (documented, no code changed): `map_reports` DDL diverges (report-location.php 3 indexes + self-heal ALTER vs edit/reports/locations.php 2 indexes — but the writer's ALTER already self-heals it at runtime, so the gap is cosmetic); `map_feature_locks` byte-identical in 2 files (DRY candidate, M4-ish); `map_feature_relations`+`map_proposals` dead (sql-only, 0 PHP refs) → DEAD section, **verify live DB before any DROP**; ⚠️ core tables (`users`/`map_features`/`map_revision`/`map_audit_log`/`location_reports`/`political_territory`) have NO inline DDL → a fresh DB must be seeded from sql/. ✅ follow-up done: aligned the divergent `map_reports` index in code — the reviewer endpoint (`edit/reports/locations.php`) now also creates `idx_map_reports_ip_hash_created_at`, matching the writer (`report-location.php`); both CREATEs now have 3 indexes (the writer's ALTER already self-healed it at runtime, so this is consistency + belt-and-suspenders for the reviewer-only edge case). ⏳ deliberately NOT done (assessed): DRY the `map_feature_locks` duplicate — it's the **intentional self-healing pattern** (each endpoint ensures its own tables independently), DRYing would couple them; drop dead schema (`map_feature_relations`/`map_proposals`) — needs a **live-DB check** first (they're sql-only); `sql/migrations/` — future structure, no migrations pending. Original scope: `sql/schema.sql` baseline, document inline-DDL tables, `sql/migrations/`, drop dead schema (`map_feature_relations`, `map_proposals`), reconcile `map_reports` divergence; **keep self-healing inline ensure-tables** |
| **M8** | Docs & i18n | in progress | ✅ "no backend" claims fixed at source (`bc175741`: llms.txt/site-summary/README). ✅ **docs → English (`94f7d277`):** the 12 authoritative German docs + `api/README.md` translated (domain terms, literal in-app UI labels, quoted console/help strings, and all code/JSON examples kept verbatim per §8); also dropped 2 dangling cross-refs in `map-features-rest-architecture.md` to docs deleted in M0. Already-English docs (refactoring-masterplan, m4-valid-to-bf-findings, stabilization-smoke-test, political-territory-editor, stylized-map-tiles) left as-is. ✅ deleted 5 superseded pre-masterplan planning/status docs (api-restructure-plan [reorg done = M3], refactoring-{status,todo} [superseded by this tracker], css-refactoring-plan, boundary-rendering-canvas-overlay-plan) — DCE, owner-approved; also removed the now-dangling `refactoring-status.md` link in `map-features-rest-architecture.md`. ✅ public root README + llms.txt + site-summary translated to English (owner-approved; URLs/facts left verbatim — flagged github.io-vs-avesmaps.de and a residual "No third-party APIs" note for owner). ✅ `?lang=en` i18n overlay v1 shipped (planner core): engine `js/app/i18n.js` + table `js/app/i18n-en.js`, keyed `data-i18n` + `tr()`, German stays the inline default, `lang` stripped from share links (spec `docs/i18n-overlay-design.md`, plan `docs/i18n-overlay-plan.md`). ✅ transport section translated (`930811ee`: heading / Land-Fluss-Meer filters / select aria-labels + all 11 native `<option>`s; the per-JS custom combobox auto-picks up the English option text because the overlay now also runs **synchronously on load** before `map-features.js` builds the combobox; `value=` slugs untouched → routing unaffected). ✅ URL-fix (`4975d26e`): `syncPlannerStateToUrl` preserves `?lang=en` so the choice survives reload (still no localStorage; share links still strip it). All live-verified in-browser. ✅ **full public frontend translated** (`a00f5cc2`→`404dd1d2`, owner-requested, all browser-verified): context menu, view-mode switcher, spotlight + result-type badges (dynamic-key lookup wraps), popup chrome, report dialog, location reviews, the legal/`Hinweise` dialog (English legal text; German remains authoritative; trademarks/proper nouns verbatim), location-class toggle tooltips + settlement labels, and the `Meilen` unit — ~165 strings; German default stays fail-safe, domain content (place names, slugs, BF) untouched. A repo-wide key-consistency check (236 table keys, 0 missing literals) guards the table. The editor / WikiSync / admin surface stays German by design. ⏳ remaining: internal API `message` strings → English (resolvable client-side via `tr()` keyed on `error.code`, per spec §10). |
