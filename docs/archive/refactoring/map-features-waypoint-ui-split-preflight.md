...
- `js/routing/routing.js` (`collectAndValidateSelectedLocations`)
...
- `js/routing/routing.js` (Live-Update/Polling-Pfad)
...
- `js/routing/routing.js` (`#inputLocation`-Click-Handler)
...
- `js/map-features.js` (Definition)
- `js/routing/routing.js` (Popup-Action `add-location-to-route`)

### `resetWaypointInputs`
- `js/map-features.js` (Definition)
- `js/routing/routing.js` (Init-Flow)
...
- `js/map-features.js` (Definition)
- `js/map-features.js` (`removeWaypointById`)
- `js/routing/routing.js` (delegierter `.remove-waypoint`-Click)

### `removeWaypointById`
- `js/map-features.js` (Definition)
- `js/routing/routing.js` (Popup-Action `remove-waypoint`)
...
## 4. Welche Funktionen von `js/routing/routing.js` genutzt werden
Direkt in `js/routing/routing.js` verwendet:
- `getWaypointContainers`
- `refreshPlannerAfterFeatureChange`
- `appendWaypointInput`
- `fillLastEmptyWaypointOrAppend`
...
Im Codebestand (JS/HTML) wurden keine weiteren externen Aufrufer ausser `js/routing/routing.js` und `js/map-features-layer-state.js` gefunden.

## 7. Welche Top-Level-Initialisierungen betroffen sind
Betroffene Top-Level-/Init-Pfade:
- `js/routing/routing.js` `routeDataRequest.then(...)`:
  - `initializeWaypointSorting()`
  - `#inputLocation`-Binding -> `appendWaypointInput()`
  - `resetWaypointInputs()`
  - `applyPlannerStateFromUrl()` (indirekt Waypoint-Funktionen)
- Delegierte Events in `js/routing/routing.js`:
...
- `#search`-Input/Change-Handler in `js/routing/routing.js` synchronisieren URL-State und greifen damit indirekt den Waypoint-Zustand auf.
...
Damit stehen die Waypoint-Globals fuer `map-features-layer-state.js`-Laufzeitpfade und fuer `js/routing/routing.js` sicher bereit.
