...
- Freie Kartenlabels (`js/map-features-labels.js`): keine direkte Funktionsabhaengigkeit im Cluster.
- Ortsnamenlabels (`js/map-features-location-name-labels.js`): direkte Stilabhaengigkeit ueber `getLocationNameLabelSize("dorf")`.
- Kraftlinienlabels (`js/map-features-powerlines.js`): keine direkte Abhaengigkeit im untersuchten Cluster.
...
Bewertung:
- `js/map-features-path-labels.js`: risikoaermer fuer einen engen, separaten 1:1-Extract des Textlabel-Clusters.
- In einem spaeteren groesseren Path-Cluster (`js/map-features-paths.js`) belassen: ebenfalls moeglich, aber groesserer Scope und hoehere Koppelungsflaeche.

Fuer den aktuellen klassischen Script-Tag-Aufbau ist **`js/map-features-path-labels.js`** der risikoaermere erste Schritt.
...
5. `js/map-features-path-labels.js` (neu)
6. `js/map-features.js`

Begruendung:
- `getPathLabelStyle(...)` nutzt `getLocationNameLabelSize(...)` aus `js/map-features-location-name-labels.js`.
...
- Minimaler spaeterer Extract:
  ...
  - `refreshPathLayerText`
  - `syncPathLabels`
  nach `js/map-features-path-labels.js`.
