...
## Neunter Ausbau: Wiki-Wappen übernehmen und lokal aktivieren

Der echte Menüablauf führt zuerst `bulk_record_coats`, danach `localize_coats` aus. Beide schreiben jetzt je Anfrage einen atomaren Sammelbeleg mit dem angemeldeten Akteur. Die Zuordnung ist auf 200 Orte begrenzt (bisher maximal 500; der vorhandene Client verwendet unverändert 150), die lokale Aktivierung unverändert auf 40 (Client: 10). Eigene Uploads, `coat_none` und die Gemeinfreiheitsprüfung bleiben geschützt. Ein bereits lokalisiertes Wiki-Wappen mit gleicher Herkunft wird nicht erneut auf seine externe Adresse zurückgesetzt; dadurch bleibt eine Wiederholung ein No-op.

**Die beiden Phasen sind getrennte Belege, keine gemeinsame Gesamttransaktion.** Zurücknehmen erfolgt in umgekehrter Reihenfolge: erst die lokalen Aktivierungen, dann die Zuordnungen. Wiederherstellen erfolgt vorwärts. Ein Versuch, einen früheren Beleg vor den späteren Änderungen zurückzunehmen, bricht vollständig ab. Der bestehende Schutz vor fremden Sperren, veralteter Planung und späteren Eigenschaftsänderungen gilt für beide Schreiber. Bestehende Namen und Geometrien werden nicht zurückgesetzt.

Downloads und Bildverkleinerung erfolgen außerhalb der Transaktion. Die erfolgreich geladenen Mitglieder werden anschließend gemeinsam geschrieben; fehlgeschlagene Downloads bleiben wie bisher gezählt und offen. Bei ausschließlich fehlgeschlagenen Downloads entstehen weder Revision noch Beleg. Ein Konflikt bei der anschließenden Speicherung verwirft das gesamte geplante Schreibpaket. Bereits erzeugte Bilddateien bleiben Cache-Dateien und sind ausdrücklich kein rücknehmbarer externer Effekt.

Neue lokale Dateien tragen Kennung und vollständigen SHA256-Inhaltshash. Ein temporärer Dateischreibvorgang veröffentlicht nur vollständig geschriebene, ausdrücklich mit `0644` lesbar gemachte Dateien. Andere Bildinhalte erhalten andere Dateinamen; ältere Belege behalten damit ihre Bilder. Undo/Redo verändern die Zuordnung, löschen aber keine Dateien und laden nichts nach. Die automatisch folgende Wappenprüfung verwendet bei lokalen Kopien die gespeicherte Wiki-Herkunft, einschließlich `Spezial:Dateipfad`-Adressen. Ohne Herkunft ist ein technischer lokaler Name kein Löschbeleg.

Abnahme: Zuordnung mit 1/27/200 und 201 Orten in zwei Anfragen; lokale Aktivierung mit 1/27/40; vollständige Folge Zuordnung → Aktivierung → beide Rücknahmen → beide Wiederherstellungen. Eigene/entfernte/unfreie/unveränderte Wappen, Wiederholung, frühere Wappen, Fehler am letzten Write, Snapshotgrenze, spätere Lokalisierung, Bearbeitung während des Downloads, Teilfehler und vollständig fehlgeschlagene Downloads geprüft. Reale Dateiinhalte und Linux-Modus `0644` sowie Erhalt alter Bilder bei neuen Inhalten sind geprüft. Ein frischer MySQL-Prozess mit den tatsächlichen Endpunkt-Abhängigkeiten prüft den ersten Bulk-Aufruf. MySQL-Parallelprobe: unbeteiligte Kartenzeile bleibt frei, während des Wartens gesetzte Editorsperre verhindert jeden Write.

Browserprüfung auf lokaler MySQL-Fixture: Originalverlauf und Original-Infopanel, vereinfachte Ortsdarstellung mit echten lokalen Bilddateien. 27 Wappen übernehmen/aktivieren, Verlauf aufklappen, beide Schritte rückwärts zurücknehmen und vorwärts wiederherstellen. Das rote neue Bild, fehlende lokale Aktivierung und blaue Vorgängerbild wechseln unmittelbar ohne Neuladen; Hell/Dunkel geprüft. Der echte Wiki-Download wird in dieser Fixture durch lokale Bildbytes ersetzt. Das gesamte Workflow-Testfeld umfasst 454 PHP- und 641 JS-Dateien; lokal bleibt nur der bekannte externe DNS-Linkcheck rot. Beide Prüfagenten haben den abschließenden Diff freigegeben.
