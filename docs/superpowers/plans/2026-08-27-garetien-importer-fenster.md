...
<div id="garetien-importer" class="gi-win" role="dialog" aria-label="Garetien Importer" hidden>
	...
	<!-- 💣 Das Menueband ist ZWEISTUFIG, und die Stufen teilen sich die Arbeit:
	     `.avm-ribbon-bar` traegt Hintergrund, Innenpolster und die Trennlinie nach unten,
	     `.avm-ribbon` NUR das Raster (css/components/editor-body.css). So machen es beide
	     echten Editoren (html/wege-editor.html:55, html/landschaften-editor.html:64).
	     🪰 Das Mockup zeigt hier nur EIN `.avm-ribbon` -- es ist eine autarke Datei und hat
	     sich die Polsterung in dieselbe Regel geschrieben. Wer das abschreibt, bekommt ein
	     Menueband ohne Grund und ohne Trennlinie, und es faellt erst auf, wenn Kacheln
	     darinstehen. Dieselbe Falle wie bei der abgeschriebenen Listenzeile (AGENTS.md §11). -->
