...
> der volle Landschaftseditor mit drei Spalten ist **nicht** Teil dieses Vorhabens.
> Diese Aufgabe ist das Minimum, das V3 sein Fertigkriterium („eine **benannte** Fläche")
> überhaupt erreichbar macht — der herausgelöste, unverzichtbare Kern davon.
>
> ⚠️ **Berichtigt 2026-07-27.** Hier stand „(V6)". Der dreispaltige Editor hieß einmal V6,
> dann wurde der Name für die Wiki-Zuweisung wiederverwendet. Der Editor hat seinen eigenen
> Plan: `docs/superpowers/plans/2026-07-27-landschaften-editor.md`.
...
| | Vorhaben | Bemerkung |
|---|---|---|
| **V5** | Kachel-Ableitung Land/Wasser | ✅ **gebaut 2026-07-27**, Werkzeug in `tools/ecosystem/`, Plan `docs/superpowers/plans/2026-07-26-landschaften-v5-kachel-ableitung.md`. **124 Flächen** abgeleitet (82 `insel` + 42 `see`), Median 32 Ecken / p90 67 / max 116 — unter der Baronie-Dichte. 🔧 Der Import selbst ist owner-getriggert. 🔴 **Nicht 149:** `kueste` (2), `wueste` (4) und `kontinent` (2) sind mit einer Land/Wasser-Schwelle **nicht** ableitbar und bleiben Handarbeit; dazu 6 umstrittene Formen (Archipele unter Einzelnamen) und 5 Label ohne Komponente. `meer` (35) und `gebirge` (60) waren nie Teil von V5 — sie gehören in eigene Vorhaben bzw. V8. ⚠️ **„gebirge (60)" zählt LABELS, nicht Flächen** (live nachgezählt 2026-07-28: **68** Labels, **2** Flächen). Dieselbe Verwechslung stand in der V8-Zeile und in `oekosystem-instruction.md` §4.2. |
| **V6** | Wiki-Regionen an Flächen zuweisen | ⚠️ **Der Name V6 meint hier NUR die Wiki-Zuweisung, nicht den Editor.** Ursprünglich war V6 der dreispaltige Landschaften-Editor; der Name wurde für die Zuweisung wiederverwendet, die bekam ihr ✅ — und der Editor blieb ohne Eintrag zurück und rutschte durch. Er ist **nicht abgewählt worden**: er ist der **siebte Listen-Editor** und hat seinen eigenen Plan, `docs/superpowers/plans/2026-07-27-landschaften-editor.md` (gebaut 2026-07-27). Wer unten „V6 ✅" liest, liest die Zuweisung. ✅ **gebaut 2026-07-27**, Instruction `docs/superpowers/plans/2026-07-27-landschaften-v6-wiki-zuweisung.md`. 🔴 **Die Schätzung „1.800–2.600 Z., 3-Spalten-Editor, zwei Sitzungen" war überholt und ist gestrichen.** Es wurde **kein** eigener Editor: die Zuweisung sitzt in der vorhandenen Liste `WikiSync → Regionen` (Flächen-Chips je Zeile + ein Zuweisen-Dialog), weil der teure Teil längst gebaut war — der Join-Schlüssel (`avesmapsEcosystemWikiRegionKey`, wortgleiche Abschrift von `avesmapsPoliticalSlug`), die Liste selbst, das Trockenlauf-Muster und die 1:n-Beziehung `ecosystem_area.region_id`. Tatsächlich **~640 Z. in einer Sitzung**, davon eine neue Datei (`js/review/review-region-sync-ecosystem.js`) und zwei Endpunkt-Aktionen. **Zuweisen setzt `wiki_url`**, den Schlüssel leitet der Server ab; mehrere Regionen dürfen denselben tragen (`idx_ecosystem_region_wiki` ist Index, nicht UNIQUE) — so erfüllt sich „eine Wiki-Region auf mehrere Flächen" ohne Verschmelzen, Verschieben oder Löschen. |
| **V6b** | Eigenschaften einer Fläche ändern | ✅ **gebaut 2026-07-27** (`18d11cda`), Verhalten in `docs/oekosystem-editor-verhalten.md` §7b. **Keine eigene Instruction** — nachgetragen 2026-07-28. Rechtsklick auf die Fläche → „Eigenschaften …" (`js/map-features/map-features-ecosystem-properties.js`): Name, Auto-Name, „Regionname anzeigen", Nodix, Art und die Wiki-Landschaft, dazu Löschen. ⭐ **Der Dialog ist der erste Aufrufer von `update_region`/`delete_region`** — beide standen seit V2.3 ohne einen einzigen: Name, Art und Wiki-Link ließen sich nur beim Anlegen setzen, und eine Region war überhaupt nicht löschbar. Speichern trägt **Name, Art, Nodix und die Wiki-Landschaft** ans Label weiter; Größe, Rotation, Zoom-Band und Priorität gehören dem Label allein. 💣 **Die Wiki-Zuweisung wandert NUR abwärts** — hat die Region keine, bleibt das Label unangetastet, sonst löschte jedes Speichern einer wiki-losen Region genau das, was V6c von Hand gesetzt hat. 💣 **`wiki_url` reist nur mit, wenn der Link angefasst wurde** (`update_region` schreibt nur, was im Payload steht). 💣 **Der `label_public_id`-Zeiger kann ins Leere zeigen** — es entscheidet das Label, nicht der Zeiger, ob beim Wiederanhaken eins entsteht. |
| **V6c** | Labels an Wiki-Regionen zuweisen | ✅ **gebaut 2026-07-27**, Backend `da28f691`, UI `9f6bfc15`; Verhalten in `docs/oekosystem-editor-verhalten.md` §7c. **Keine eigene Instruction** — nachgetragen 2026-07-28. Das Spiegelbild von V6 für **Karten-Labels**: derselbe Listeneintrag `WikiSync → Regionen` trägt neben „Fläche zuweisen" den Knopf **„Label zuweisen"**, derselbe Ablauf auswählen → Vorschau (Trockenlauf) → Zuweisen. `avesmapsWikiRegionAssignLabels` (`api/_internal/wiki/regions.php`), Aktion `assign_labels` in `api/edit/wiki/regions.php`, Dialog in `js/review/review-region-sync-ecosystem.js` (700 Z., geteilt mit V6), Unit-Test `api/_internal/wiki/__tests__/region-label-assign-test.php`. 🔴 **Bewusst NEBEN `avesmapsWikiRegionAssign`, nicht darin:** jene Aktion matcht ausschließlich über den NAMEN und trägt den „⛰ Berge zuordnen"-Bulk — ein Label, das anders heißt als seine Wiki-Region, ist dort gar nicht erreichbar, und Umbauen hätte den Bulk mitgerissen. ⚠️ **Ein Label-Save bumpt `map_revision`, eine Flächen-Zuweisung nie** — Labels reisen im `map-features`-Payload, Flächen nicht; der Bump passiert **einmal je Aufruf**, nicht je Label. Wer das als Verstoß gegen Regel 3 „korrigiert", bricht die Kartenaktualisierung. Den Typ-Konflikt-Wächter des Bulks wiederholt der Dialog absichtlich nicht (hier wählt ein Mensch); **Lösen** bleibt Sache des Label-Editors, ein leerer Schlüssel wird abgewiesen. |
| **V7** | Grenzimport aus den Territorien | ✅ **gebaut UND live abgenommen 2026-07-28**, Instruction `docs/superpowers/plans/2026-07-28-landschaften-v7-grenzimport.md`, Verhalten in `docs/oekosystem-editor-verhalten.md` §4a. Rechtsklick → „Grenze aus Territorien …" → Hierarchiebaum mit Häkchen → vereinigen und einfügen (**Kopie, nie Verknüpfung**), danach öffnet der **vorhandene** Vereinfachen-Dialog (Owner-Entscheid 3 — nicht ein zweiter hier). Ebene fragt der Dialog, vorausgewählt derographisch (Entscheid 1); genommen wird, was der Layer zeigt (Entscheid 2), jede Zeile sagt „Außengrenze"/„Grenze". Nachgemessen im Browser: 946 Gebiete öffnen = **9 ms**, ein ganzes Reich (64 Gebiete) vereinigen = **29 ms**, filtern = **3 ms**; 56 Baronien = 2240 rohe Ecken → 552 vereinigt → 150 bei Regler 75. 🔴 **`round(…,4)` halbiert die Nutzlast NICHT — gemessen −33 %** (15,0 → 10,1 KB); wie viel es wird, hängt an der Stellenzahl der Quellkoordinaten. *(„Senden an …" ist nach V3.6 vorgezogen; ein Massen-`copy_regions` ist durch V4 **gestrichen**.)* |
| **V8** | Topographie / Höhenfeld | ✅ **gebaut 2026-07-28**, Instruction `docs/superpowers/plans/2026-07-28-landschaften-v8-topographie.md`. Die Höhe wohnt als `height_schritt` in `properties_json` des `berggipfel`-Labels (kein neuer Endpunkt: `update_label` nahm `properties_json` schon), eingetragen in **beiden** Oberflächen. Gipfel sind in der Topographie-Ebene sichtbar und **dauerhaft ziehbar** — kein neues Modul, der vorhandene Label-Marker *ist* der Gipfel. 💣 **`sampleRoute()` :637 nicht übernommen** (gehört zu V11). 🔴 **Zwei Behauptungen dieser Zeile waren falsch und sind berichtigt:** (1) „Enthaltensein-Fensterung statt `max` oder Summe" — **gestrichen**, Owner-Entscheid 2026-07-28: Flächen **summieren** sich, ein **globales Gipfelfenster** hält jeden benannten Gipfel auf seiner eigenen Zahl; kein Baum, kein Enthaltensein-Test. (2) „34 Gipfel auf 61 Gebirge" — live gezählt sind es **62 `berggipfel`-Labels**, **68 `gebirge`-LABELS** und **2 `gebirge`-FLÄCHEN**; die alte Zahl zählte Labels und sagte Flächen. (3) Die **Gipfel-Sichtbarkeitsregel ist abgewählt** — kein Gipfel verschwindet von der Standardkarte. **Gemessen:** Feld je Fläche 3,7 ms, Neuzeichnen 36 ms bei 900×600; Verfeinern verändert das Gelände um 3,8 % statt um 85 % (die Dämpfung liegt jetzt hinter allen Stufen, nicht nur hinter der groben). 🔴 **Offen:** V8 baut das Werkzeug — die ~66 fehlenden `gebirge`-Flächen sind Redaktionsarbeit, und die V5-Kachelableitung greift bei Gebirgen nicht. |
| **V9** | Vorberechnung Wege × Flächen | ✅ **gebaut UND live abgenommen 2026-07-29**, Spec `docs/superpowers/specs/2026-07-29-landschaften-v9-vorberechnung-design.md`, Plan `docs/superpowers/plans/2026-07-29-landschaften-v9-zugehoerigkeit.md`. 🔴 **Der Zuschnitt der Zeile unten ist überholt** — Owner-Entscheid: gerechnet wird im **Browser**, am vorhandenen Knopf „Zugehörigkeit rechnen" des Landschaften-Editors, der dort schon zwei Zuordnungen rechnete und wegwarf. **Gemessen 0,4 s** für 9.091 Zeilen (668 Flächen × 5.655 Wege) — damit entfielen Stapellauf, Sperre, Budget, Cursor und `path_ecosystem_state` **ersatzlos**; aus 5.655 Zustandszeilen wurde **eine** Stempelzeile. Erst messen, dann bauen. `path_ecosystem` hat PK `(path_id, area_id, **basis**, seq)` — `basis` 0=Sehne / 1=gezeichnete Catmull-Kurve, weil beide **verschiedene Intervallmengen** liefern (6 Paare nur Sehne, 4 nur Kurve) und die öffentliche Karte **null Polygone** lädt, die Kurve also nicht zur Anzeigezeit ableiten kann. `affects_paths` je Art: meer/kontinent/kueste aus, sie machten 90 % der Arbeit für die Aussage „führt durch Aventurien". ⭐ Der Kern nimmt eine **Koordinatenliste**, kein Weg-Objekt — V13 ist damit derselbe Aufruf mit zwei Punkten. |
| **V10** | „Führt durch" + Flora am Segment | ✅ **gebaut UND live abgenommen 2026-07-29**, Spec `docs/superpowers/specs/2026-07-29-landschaften-v10-fuehrt-durch-design.md`, Plan `docs/superpowers/plans/2026-07-29-landschaften-v10.md`. **Vier** Anzeigeflächen statt der einen dieser Zeile: Routen-Zusammenfassung, Etappen**zeile** im Planer, Etappen-Infobox, Weg-Infobox. 🔴 **Zwei Tonlagen, EIN Rechner** (Owner: „beim routenplaner muss kein % dranstehn"): der Planer erzählt (blanke Namen, `durch:` im Fliesstext, und **nur was gegenüber der Zeile darüber neu ist** — aus 31 fast gleichen Zeilen werden 9), die Infobox belegt (Anteile, `·`, vollständig). `buildLandscapeLine` rechnet, drei dünne Schreiber setzen. Lesepfad `POST /api/app/path-landscapes.php` bei `basis=0`; **eine** Anfrage je Route, faul je Weg-Infobox, **null** beim Kartenaufbau (live nachgezählt). Flora+Fauna **ohne Waren** über `data-lore-kinds` am Container — `AVESMAPS_LORE_ROWS` bleibt unangetastet, sie speist auch die Siedlung. 💣 **Die Zeile dieses Plans war in einem Punkt falsch und hat live einen Totalausfall verursacht:** eine Etappe trägt in `properties.id` die **Kanten**-Kennung `path-2661`, nicht die `public_id` — das gilt nur unter `?clientrouting=1`, live läuft `shouldUseServerPrimaryRouting()`. Der Server lehnte ab, der Client schluckte es, und die leere Zeile sah **genau wie das richtige leere Ergebnis** aus. Behoben + bewacht (`js/routing/__tests__/route-entry-path-ids.test.js`, gegen den alten Stand gegengeprüft). 💣 Anteile summieren sich **nicht** auf 100 (drei überlagernde Ebenen), und eine Landschaft kann an einer Etappe stehen und in der Routenzeile fehlen — anderer Nenner. ⚠️ Nur über den DOM-Observer laden, nie beim Markup-Bau. 🔧 Nach jeder Flächenänderung „Zugehörigkeit rechnen" drücken, sonst zeigt die Zeile den älteren Stempel. |
| **V11** | Terrain auf Kantengewichte | ✅ **gebaut und abgenommen 2026-07-30**, Schalter `terrain_travel_enabled` **AN**. Instruction `docs/superpowers/plans/2026-07-29-landschaften-v11-gelaende-reisezeiten.md`, Messungen + Abnahme `…-v11-messung.md`. Wirkung Gareth→Thorwal **+0,89 %**, `terrain:false` liefert die Grundlinie `65.60202533378045` bit-identisch. Die Rückrechnung `$originalDistance / $originalTime` wurde **nicht repariert, sondern unnötig gemacht** (Elternfaktor herausrechnen, eigenen anwenden). Einheitenfalle vermieden, Steigungsmodell seit 2026-07-30 der **Leistungskilometer** (DIN 33466): `Meilen + Aufstieg/100 + Abstieg über 20 % Gefälle/150`, Deckel 4,0, **keine Untergrenze mehr** — nichts ist je schneller als die Ebene, Owner-Entscheid 3 ist damit gefallen. Die abgelöste Kurve `1+5·auf−1,5·ab+3·ab²` und Tobler unterstellten 490 bzw. 466 Hm/h Steigleistung, die Normen sagen 300–400 (§4 der Messdatei). Klemme 4,0 (**nicht** die Flussgrenze), `from`/`to` bleiben in gespeicherter Orientierung. **Owner-Entscheide 2026-07-30:** Klemme 4,0 bleibt (trifft 0 von 2.493 Land-Wegstücken, beide Richtungen; Sättigung nur 21 % entfernt) · **Wasserwege ausgenommen** (`avesmapsRouteAttachTerrain`, Verbotsliste — ein Fluss ist schon über die Strömung bepreist, und das steilste Stück der Karte WAR ein Fluss) · Rechnung im Tempo-Dialog offengelegt. 🔴 **Offen und Redaktionsarbeit:** der 2,67×-Anker der Tempotabelle wird von den Daten **nicht** getragen (5 von 4.300 Stücken erreichen ihn, ein typischer Gebirgspass liegt bei 1,0000), weil 15 der 16 Gebirge auf der Platzhalterhöhe laufen. Erst danach `UP_PENALTY` + Klemme neu ansehen. ⚠️ Zwei Abnahmeschritte eingeschränkt: Wegpunkt-Anker nur halb sichtbar (nur `-a` liegt auf der Route), Überlappungsstreifen heute gar nicht prüfbar (**0 von 16** Gebirgspolygonen überlappen). |
| **V12** | Geschwindigkeitsvektoren | ✅ **gebaut 2026-07-30**, aber **enger zugeschnitten als diese Zeile es vorsah** — Owner-Entscheid: nicht über die ganze Karte, sondern **nur auf der geplanten Route im Editmodus**, Haken „Geschwindigkeit" in den Routenoptionen (`js/routing/route-speed-arrows.js`, Muster `map-features-river-flow-arrows.js`, Canvas-Pane 455, 52 px Abstand). ⭐ **Der Zuschnitt löst zwei Probleme, die die Karten-weite Fassung erst geschaffen hätte:** eine geplante Route TRÄGT ihre Faktoren schon in der Antwort (über die ganze Karte bräuchte es einen neuen Leseweg für `path_terrain` — es gibt keinen), und die Wasserregel ist darin bereits angewandt (ein eigener Leser fände die noch liegenden Wasserzeilen). Dazu wären über die ganze Karte **97 %** der Wegstücke gleichförmig — gemessen tragen nur 3,05 % einen Faktor ≠ 1. 💣 **Die LÄNGE ist die Aussage, und sie verträgt keinen Sockel:** erst `7 + 14·rel` plus gefülltes Dreieck (Breite wuchs mit) → unlesbar; dann ein 5,4-px-Saum mit runden Kappen, der jedem Pfeil dieselbe Grundlänge anhängte. Jetzt Schaft + **feste** Spitze, Länge proportional zur relativen Geschwindigkeit (gemessen 12/16/34/40 px bei Faktor 3,48/2,20/1,0/0,86). 💣 **Und die Pfeilrichtung ist NICHT die Koordinatenreihenfolge** — 11 von 45 Etappen sind rückwärts gespeichert; die Fahrtrichtung folgt aus der Verkettung, siehe `orientToTravelDirection()`. Der Faktor selbst kommt vom Server (`terrain_time_factor` durchgereicht, keine Kurve im JS). ⚠️ Nicht reaktiv: zeigt, was der letzte Profillauf gerechnet hat — so gewollt. 🔴 **Der „Versatz senkrecht zur Segmentrichtung“ dieser Zeile gehörte zur BIDIREKTIONALEN Karten-Fassung und wird für die Route NICHT gebraucht** (Owner, 2026-07-30): ein Weg trägt zwei Richtungsfaktoren (`factor_forward`/`factor_backward`, `docs/oekosystem-instruction.md:86`), also zwei Pfeilreihen, die sich ohne Versatz überlagern würden — eine Route durchläuft jede Etappe genau einmal. Auch der Prototyp hat keinen (`html/landschaften-modell.html:708` zeichnet vom Punkt aus). 🔴 **UND DIE KARTEN-WEITE, BIDIREKTIONALE FASSUNG IST GESTRICHEN** (Owner, 2026-07-30): *„die Route ist jederzeit verfügbar und hilft beim Visualisieren, die Pfeile wären ohnehin nicht responsiv gewesen."* Über die ganze Karte hätten sie überall den Stand des letzten Profillaufs gezeigt — ohne den Anlass, der auf einer gerade geplanten Route mitkommt. Der Versatz entfällt damit ebenfalls. **V12 ist abgeschlossen.** Offen bliebe allein eine Legende für den Längenmaßstab, ~20 Zeilen, bewusst nicht gebaut. |
| **V13** | Querfeldein: Wasser meiden | ✅ **gebaut UND live abgenommen 2026-07-30**, Spec `docs/superpowers/specs/2026-07-29-landschaften-v13-wasser-design.md` (kein eigener Plan — Owner-Entscheid „erst messen, dann entscheiden", die Messung *war* die Planung). Schneidet die Sehne ein `meer`/`see`-Polygon, entsteht die Kante nicht; gibt es eine trockene Ausweichverbindung, wird sie genommen, gibt es keine, entsteht **gar keine** Kante und der Planer sagt „keine Route gefunden" (Owner: *„Wirklich weglassen."*). **Inline im Server, bei jeder Anfrage** — kein Knopf, kein Stempel: 💣 es gibt **keine stabile Komponente zum Stempeln**, sie hängt an der Transportauswahl (896 Komponenten mit allen Verkehrsmitteln, **1.201** nur zu Fuß). 🔴 **Zwei Behauptungen dieser Zeile waren falsch:** (1) „im Client" — live läuft `shouldUseServerPrimaryRouting()`, gebaut wurde in `client-graph.php`; `USE_SERVER_ROUTING = false` ist ein zweiter, **toter** Schalter, den nur `calculateRouteByMode()` liest. (2) „~50 Z." — es wurden 5 Dateien und ~420 Produktionszeilen plus 3 Testdateien, weil Küstentoleranz, Deckel und der **zweite** Erzeuger aus Fahrplanhöhe unsichtbar sind. **Live gemessen 2026-07-30** (`map_revision` 46722, `ecosystem_revision` 5609): `synthetic_connection_count` **901 → 861**, also **−40**; der Offline-Nachbau traf die Live-Zahl **bitgenau** (4675 Knoten, 5676 Wege, 861 — alle drei identisch), Aufpreis **+70 ms** auf 791 ms Graphaufbau. Netzlauf: Gareth→Retohaven `found:false` (vorher eine Querfeldein-Etappe über See), Gareth→Kuslik `found:true` mit 44 Segmenten. 💣 **571 von 4.653 Orten (12,3 %) liegen geometrisch IM Wasser**, 55 benannte — Belhanka, Kuslik, Salzerhaven, Neersand, Yaisirabad: Häfen werden auf die Küste gezeichnet, das Meerpolygon großzügig darüber. Ohne die **Küstentoleranz 1,0** brechen **51 von 85** Fällen fälschlich (kurze Küstenhüpfer); ab 5,0 fallen echte Überfahrten durch — die Verteilung hat **keinen natürlichen Sprung**, der Wert ist Ermessen (sichere Spanne 0…3,0). 💣 **Die alte `SEA_CROSSING_MIN_DISTANCE = 20.0` war im Normalbetrieb TOT** (0 von 896 — eine Komponente mit Seeweg hängt über genau diesen Seeweg schon am Hauptteil und ist nicht abgehängt) und ist **entfernt, nicht repariert**; ⚠️ `avesmapsClientNodeIsWaterLocked` **BLEIBT** (andere Frage: nur per Schiff erreichbar — mitentfernen ließe Landbrücken auf Inselhäfen zu). 💣 **Unbegrenzte Ausweichsuche kostet 17–47 s**; gefunden wird früh (Median Rang 3, max 16), **Deckel 25** kauft alles, 250 nichts mehr. 💣 **ZWEI Erzeuger** — Komponentenbrücke *und* Wegpunkt-Anker; nur den ersten zu sichern lässt die Sperre halb gebaut, genau an den Orten, die der Nutzer selbst eingibt. ⭐ **Keine zweite Mathematik:** der V9-Kern nimmt eine Koordinatenliste, portiert nach `api/_internal/app/ecosystem-line-intervals.php`; Parität über **eine geteilte Fixture, die beide Testsuiten lesen** (18 Fälle) plus **4.000 Live-Sehnen bitgleich** mit dem JS-Original. ⭐ Leeres Wasser = V13 **inert**, Planer verhält sich exakt wie vorher — das ist der gebaute Ausfallweg, keine 500. ⚠️ `route.debug.context` war anonym **lesbar** (die Warnung, es hänge an der `edit`-Berechtigung, traf nicht zu) — daher steht der Zähler direkt im Nachweis. 🔴 **Offen:** der **Client** (`route-graph-routing.js:49`) hat **gar keine** Wasserlogik, nicht einmal die alte Regel — eigene Sitzung, Server zuerst (Owner). |
| **V14** | A\* für Querfeldein | ✅ **gebaut UND live** — „Hierher reisen" 2026-08-01 (Instruction `docs/superpowers/plans/2026-07-30-hierher-reisen-und-astar.md`), die beiden übrigen Aufrufer 2026-08-02 (Instruction C `docs/superpowers/plans/2026-08-02-ausloeser-anker-und-x25-aufschlag.md`). 🔴 **„Nur clientseitig" war falsch** und stammt aus der Zeit vor V11 §10 — gebaut ist es **im Server**: `api/_internal/routing/{land-areas,offroad-grid,offroad-data,offroad-leg,detour,synthetic-refine}.php`, `POST /api/route/` nimmt `from_point`/`to_point` = `{x, y}`. ⭐ **„on demand, nicht vorberechnen" ist geblieben** und hat sich ein zweites Mal bestätigt: **876** synthetische Kanten je Graph, aber **0–1 je ROUTE** (fünf Referenzrouten, 2026-08-02) — deshalb läuft der A\* über die Sehnen der *gefundenen* Route, nie beim Graphbau. **Drei Aufrufer, ein Rechner:** (1) „Hierher reisen"; (2) der automatische Umweg-Auslöser (`AVESMAPS_ROUTE_OFFROAD_DETOUR_THRESHOLD` = 3,0, gemessen 18,01× / 6,83× / 3,84× / 1,13× an den vier Freigabefällen); (3) das Biegen der Notbrücken-Sehnen. 💣 **Die Schwelle entscheidet nie allein** — Querfeldein ist ~3× langsamer als eine Straße, zweite Prüfung ist die Zeit. 💣 Und der ×25-Aufschlag der Notbrücken ist ein **Dijkstra-Gewicht**, keine Strecke und keine Zeit; er wird aus jeder gemeldeten Zahl per `cost_factor` herausgerechnet. |
