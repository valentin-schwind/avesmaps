...
// Die Wiki-Landschaft einer Beschriftung: EINE Quelle, die Region (Owner 15.09.2026). Die einzige Datei, die
// `properties.wiki_region` schreibt -- Durchtrag, Ruecknahme, Anlegen, Aendern, Umhaengen, Rueckgaengig und
// der Bestandslauf gehen alle durch sie. Nur Funktionen; nichts laeuft beim Einbinden.
require_once __DIR__ . '/landschaft-wiki.php';
...
function avesmapsAssignEcosystemWikiRegion(PDO $pdo, array $payload, int $userId): array
{
    ...
    try {
        ...
        foreach ($targets as $publicId => $before) {
            ...
            // 🔴 Und die LEERE Adresse (die Ruecknahme, siehe Kopf) nimmt ihnen die Kopie (Owner
            // 03.09.2026). 🪤 Bis dahin lief dieser Zweig mit `$wikiKey = null` in den Durchtrag und
            // brach dort unter strict_types mit einem TypeError ab -- der Panel-Weg „Zuweisung
            // entfernen" antwortete also mit einem 500, nachdem die Flaeche schon geschrieben war.
            $primaerLabel = ($before['label_public_id'] ?? null) === null ? null : (string) $before['label_public_id'];
            $durchtrag = $wikiKey === null
                ? avesmapsEcosystemClearWikiRegionFromLabels($pdo, (string) $publicId, $primaerLabel, $userId)
...
function avesmapsCreateEcosystemRegion(PDO $pdo, array $payload, int $userId): array
{
    ...
    try {
        ...
        // 🔴 EINE QUELLE, DIE REGION (15.09.2026): die frisch gebundene Beschriftung traegt ab hier den Artikel
        // der Region -- auch keinen. Bis dahin zog dieser Weg das Label nicht nach, und der Garetien-Import
        // legte es mit einem Nest aus dem Staging an, waehrend die Region ihren Schluessel aus der Adresse
        // ableitete: zwei Ableitungen desselben Werts. Eine NEUE Region hat nur frisch gebundene
        // Beschriftungen, deshalb loescht eine leere hier (`true`).
        $labelDurchtrag = avesmapsLandschaftWikiBeschriftungenAngleichen(
            $pdo,
            $publicId,
            ($row['label_public_id'] ?? null) === null ? null : (string) $row['label_public_id'],
            (string) ($row['wiki_region_key'] ?? ''),
            (string) ($row['wiki_url'] ?? ''),
            true,
            $userId,
            'ecosystem_wiki_region_bindung'
        );
    ...
    // `labels`: die Beschriftung, die dabei ihren Artikel bekommen (oder verloren) hat, in der Form von
    // `update_label` -- dieselbe Bauform wie bei update_region.
    return [
        'region' => avesmapsEcosystemRegionSnapshot($row),
        'revision' => $revision,
...
function avesmapsUpdateEcosystemRegion(PDO $pdo, array $payload, int $userId): array
{
    ...
    // 🔴 WER EINE BESCHRIFTUNG FRISCH BINDET, GIBT IHR DEN ARTIKEL DER REGION -- AUCH KEINEN (15.09.2026, „eine
    // Quelle, die Region"). Traegt die Region einen, zieht der Durchtrag unten ihn ohnehin nach. Ist sie
    // LEER, nimmt der Durchtrag nichts zurueck (ein blosses Speichern loescht nie) -- fuer eine Beschriftung,
    // die gerade erst an diese Region kommt, waere ihr alter Artikel aber der einer anderen Landschaft.
    // „Frisch" heisst: sie hing vorher an keiner oder an einer anderen Region, gelesen VOR dem Schreiben.
    $frischGebunden = [];
    $adoptiert = avesmapsEcosystemLabelPointerToAdopt($fields);
    if ($adoptiert !== '' && $adoptiert !== trim((string) ($before['label_public_id'] ?? ''))) {
        $adoptLesen = $pdo->prepare(
            "SELECT properties_json FROM map_features WHERE public_id = :p AND feature_type = 'label' AND is_active = 1 LIMIT 1"
        );
        $adoptLesen->execute(['p' => $adoptiert]);
        $adoptRoh = $adoptLesen->fetchColumn();
        if ($adoptRoh !== false) {
            $adoptProperties = json_decode((string) $adoptRoh, true);
            $bisherigeRegion = avesmapsLandschaftWikiRegionDerBeschriftung(
                $pdo,
                $adoptiert,
                is_array($adoptProperties) ? $adoptProperties : []
            );
            if ((string) ($bisherigeRegion['public_id'] ?? '') !== $publicId) {
                $frischGebunden[] = $adoptiert;
            }
        }
    }
    ...
    try {
        ...
        $after = avesmapsEcosystemRegionRow($pdo, $publicId);
        $primaerLabel = ($after['label_public_id'] ?? null) === null ? null : (string) $after['label_public_id'];
        ...
        //
        // 🔴 UND EIN AUSDRUECKLICHES ENTFERNEN WANDERT MIT (Owner 03.09.2026, „Lawaralîr"/„Cronwald"):
        // nennt der Rumpf `wiki_url` und meint es leer, verlieren die Beschriftungen ihre Kopie --
        // sonst zeigt die Infobox weiter den Artikel, den die Flaeche gerade verloren hat. Die Weiche
        // ist die geteilte (avesmapsEcosystemWikiAusdruecklichEntfernt), keine eigene Lesart des Rumpfs.
        $labelDurchtrag = avesmapsEcosystemWikiAusdruecklichEntfernt($fields)
            ? avesmapsEcosystemClearWikiRegionFromLabels($pdo, $publicId, $primaerLabel, $userId)
        ...
        // 🔴 Die FRISCH gebundene Beschriftung an einer LEEREN Region verliert ihren alten Artikel (siehe
        // oben, `$frischGebunden`). Nur sie -- die uebrigen Beschriftungen der Region nimmt ein blosses
        // Speichern nicht zurueck.
        if ($frischGebunden !== []
            && trim((string) ($after['wiki_region_key'] ?? '')) === ''
            && !avesmapsEcosystemWikiAusdruecklichEntfernt($fields)) {
            $bindung = avesmapsLandschaftWikiBeschriftungenAngleichen(
                $pdo,
                $publicId,
                $primaerLabel,
                '',
            ...
            $labelDurchtrag['features'] = array_merge($labelDurchtrag['features'], $bindung['features']);
            $labelDurchtrag['revision'] ??= $bindung['revision'];
        }
...
// 🔴 avesmapsEcosystemWikiRegionAssignObject (der Wiki-Datensatz, den eine Beschriftung von ihrer Region
// traegt) wohnt seit dem 15.09.2026 in landschaft-wiki.php, neben dem einen Schreiber, der ihn benutzt.
// Dort traegt er einen Riegel mehr: der Schluessel des Nests IST der Schluessel der Region.
...
//
// 💣 NUR ABWÄRTS, UND NIE LÖSCHEND -- ⚠️ das AUSDRÜCKLICHE Entfernen geht seit dem 03.09.2026 seinen
// eigenen Weg (avesmapsEcosystemClearWikiRegionFromLabels, darunter). Hat die Region keinen Schlüssel, bleibt die Beschriftung
...
function avesmapsEcosystemPushWikiRegionToLabels(
    ...
    // 🔴 SEIT DEM 15.09.2026 SCHREIBT DIESE FUNKTION NICHT MEHR SELBST: sie ist die Lesart „nur abwaerts,
    // nie loeschend" des EINEN Schreibers (avesmapsLandschaftWikiBeschriftungenAngleichen,
    // landschaft-wiki.php). Name und Vertrag bleiben, weil update_region, assign_wiki_region und der
    // Bestandslauf „Wiki & Art" sie beim Namen rufen.
    if (trim($wikiKey) === '') {
        return ['labels' => 0, 'applied' => 0, 'revision' => null, 'features' => []];
    }

    return avesmapsLandschaftWikiBeschriftungenAngleichen(
        $pdo,
        $regionPublicId,
        $primaryLabelPublicId,
        $wikiKey,
        $wikiUrl,
        false,
        $userId,
        'ecosystem_wiki_region_push'
    );
}

// DIE GEGENRICHTUNG DES DURCHTRAGS: ein AUSDRUECKLICHES Entfernen an der Flaeche nimmt die Kopie an
// ihren Beschriftungen mit (Owner 03.09.2026).
//
// 🔴 DER BEFUND, am Bild: „Lawaralîr" sollte vom Wiki geloest und in „Cronwald" umbenannt werden. Die
// Flaeche verlor ihre Zuweisung, die Beschriftung behielt ihr Nest -- und weil die Infobox aus GENAU
// diesem Nest liest (labelWikiInfoboxMarkup, map-features-labels.js), standen dort weiter Lage,
// Staat und Beschreibung des alten Artikels; der Beschriftungsdialog bot an, den Namen auf
// „Lawaralîr" zurueckzuholen. Je nachdem, ob man Flaeche oder Beschriftung anklickte, war die
// Landschaft geloest oder nicht. Drei Durchtrag-Wege gab es, und alle drei waren als „nie loeschend"
// gebaut -- ein Entfernen erreichte die andere Seite deshalb auf keinem davon.
//
// 💣 „NIE LOESCHEND" BLEIBT FUER DEN RUMPF OHNE `wiki_url` -- und nur dafuer. Der Durchtrag darueber
// steigt bei leerem Schluessel weiter aus, damit ein blosses Umbenennen die von Hand gesetzte
// Zuweisung einer Beschriftung nicht loescht. Was HIER wandert, ist die AUSDRUECKLICHE Ruecknahme:
// der Rumpf NENNT `wiki_url` und meint es leer (avesmapsEcosystemWikiAusdruecklichEntfernt). Die zwei
// Faelle auseinanderzuhalten ist der ganze Inhalt dieser Funktion; wer sie zusammenzieht, baut
// entweder die Luecke von oben oder die Loeschung bei jedem Speichern.
//
// 💣 GELOESCHT WIRD NUR, WO ETWAS STEHT, und der Merker `wiki_no_article` bleibt unangetastet:
// „entfernt" heisst „nicht zugewiesen", nicht „es gibt keinen Artikel" -- das ist der dritte
// Zustand, eine eigene Aussage, die nur ein Mensch trifft.
//
// Revision, Protokoll und Feature-Rueckgabe wie beim Durchtrag: EINE map_revision je Lauf, eine
// `update_label`-Protokollzeile je Beschriftung (Grund `ecosystem_wiki_region_clear`, damit der
// Aenderungslog sie wie jedes andere update_label zuruecknehmen kann), und die geschriebene
// Beschriftung reist als Feature zurueck, damit der Browser sie sofort zeigt.
function avesmapsEcosystemClearWikiRegionFromLabels(
    PDO $pdo,
    string $regionPublicId,
    ?string $primaryLabelPublicId,
    int $userId
): array {
    // 🔴 Seit dem 15.09.2026 die Lesart „ausdruecklich entfernt" des EINEN Schreibers
    // (avesmapsLandschaftWikiBeschriftungenAngleichen, landschaft-wiki.php): leere Region, alle
    // Beschriftungen verlieren ihr Nest. Name und Vertrag bleiben fuer die Aufrufer.
    return avesmapsLandschaftWikiBeschriftungenAngleichen(
        $pdo,
        $regionPublicId,
        $primaryLabelPublicId,
        ...
        true,
        $userId,
        'ecosystem_wiki_region_clear'
    );
}

// REIN: hat dieser Rumpf die Zuweisung AUSDRUECKLICH entfernt? Nur dann wandert das Entfernen an die
// Beschriftungen. `$fields` ist das Ergebnis von avesmapsEcosystemReadRegionFields -- dort steht
// `wiki_url` genau dann, wenn der Rumpf es genannt hat, und `wiki_region_key` ist daraus abgeleitet.
// ⚠️ Ein Rumpf OHNE `wiki_url` (blosses Umbenennen) sagt ueber die Zuweisung nichts und nimmt nichts
// zurueck; ein Rumpf MIT Adresse ist eine Zuweisung, kein Entfernen. Beide Lesarten stehen HIER, in
// einer Funktion, damit kein Schreibweg sich eine eigene baut.
function avesmapsEcosystemWikiAusdruecklichEntfernt(array $fields): bool
{
    return array_key_exists('wiki_url', $fields)
        && trim((string) ($fields['wiki_region_key'] ?? '')) === '';
}

// Das FEATURE einer soeben geschriebenen Beschriftung, in der Form, die `update_label` selbst
// zurueckgibt -- damit der Browser sie mit demselben Leser (applyLabelFeaturesLocally,
// map-features-labels.js) sofort auf die Karte bringt, statt bis zum naechsten Live-Abgleich den
// alten Stand zu zeigen: den Artikel, den die Flaeche gerade verloren hat, samt rotem Halo.
//
// 🔴 DERSELBE BAUER WIE BEIM SCHREIBWEG DER LABELS (avesmapsBuildFeatureResponseFromStoredFeature,
// api/_internal/map/features.php). Eine eigene Form hier waere die zweite Wahrheit ueber ein
...
{
    $nachher = $row;
    $nachher['properties_json'] = $propertiesJson;
    $nachher['revision'] = $revision;

    return avesmapsBuildFeatureResponseFromStoredFeature($nachher);
}
...
function avesmapsEcosystemRestoreAuditRow(PDO $pdo, array $row, int $userId): void
{
    ...
    if ($action === 'create_area') {
        ...
        if ($action === 'delete_area_with_region' && $regionPublicId !== '') {
            ...
            // 🔴 Eine zurueckgeholte Region gleicht ihre Beschriftungen an (15.09.2026, eine Quelle, die Region).
            avesmapsLandschaftWikiNachReaktivierung($pdo, $regionPublicId, $userId);
        }
    } elseif (in_array($action, ['delete_region', 'delete_region_cascade'], true)) {
        $pdo->prepare('UPDATE ecosystem_region SET is_active = 1, updated_by = :u WHERE public_id = :p')
            ->execute(['u' => $actor, 'p' => $regionPublicId]);
        avesmapsEcosystemRestoreRegionLabel($pdo, $regionPublicId, $userId);
        // 🔴 Und hier ebenso: waehrend die Region stillgelegt war, konnte ihr Artikel zurueckgenommen oder eine
        // Beschriftung als freie beschrieben werden -- ohne diese Zeile stuenden beide danach auseinander.
        avesmapsLandschaftWikiNachReaktivierung($pdo, $regionPublicId, $userId);
        ...
        $regionVorUndo = avesmapsEcosystemRegionRow($pdo, $regionPublicId, false);
        ...
        // 🔴 EINE QUELLE, DIE REGION (15.09.2026): die Beschriftungen folgen dem zurueckgenommenen Stand, in
        // BEIDE Richtungen. Bis dahin stellte dieses Rueckgaengig nur die Region zurueck -- nach der Ruecknahme
        // einer Zuweisung trug das Schild weiter den Artikel, den die Flaeche gerade wieder verloren hatte.
        avesmapsLandschaftWikiNachRegionsWechsel(
            $pdo,
            $regionVorUndo,
            avesmapsEcosystemRegionRow($pdo, $regionPublicId, false),
            $userId
        );
    }
