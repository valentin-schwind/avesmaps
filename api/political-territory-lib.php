...
function avesmapsPoliticalImportWikiRecords(PDO $pdo, array $records, array $user = []): array {
    ...
    try {
        foreach ($records as $record) {
            ...
            $wikiRecord = avesmapsPoliticalNormalizeWikiRecord($record);
            if ($wikiRecord['name'] === '') {
                continue;
            }

            if ($wikiRecord['coat_of_arms_url'] !== '' && !avesmapsPoliticalIsLikelyCoatOfArmsUrl($wikiRecord['coat_of_arms_url'])) {
                $wikiRecord['coat_of_arms_url'] = '';
                $stats['invalid_coat_of_arms']++;
            }

            $wikiResult = avesmapsPoliticalUpsertWikiRecord($pdo, $wikiRecord);
            $wikiRecord['id'] = $wikiResult['id'];
            $stats[$wikiResult['created'] ? 'wiki_inserted' : 'wiki_updated']++;

            if ($wikiRecord['continent'] !== AVESMAPS_POLITICAL_DEFAULT_CONTINENT) {
                $stats['territory_skipped']++;
                continue;
            }

            $territory = avesmapsPoliticalFindTerritoryByWikiOrSlug($pdo, (int) $wikiRecord['id'], $wikiRecord['slug']);
            if ($territory === null) {
                $territory = avesmapsPoliticalCreateTerritoryFromWiki($pdo, $wikiRecord, $user);
                $stats['territory_created']++;
            } else {
                avesmapsPoliticalLinkTerritoryToWiki($pdo, (int) $territory['id'], (int) $wikiRecord['id']);
                $territory['wiki_id'] = (int) $wikiRecord['id'];
                $stats['territory_linked']++;
            }

            $imported[$wikiRecord['slug']] = [
                'territory_id' => (int) $territory['id'],
                'name' => $wikiRecord['name'],
            ];
            if (avesmapsPoliticalSeedGeometryFromMapFeature($pdo, (int) $territory['id'], $wikiRecord, $user)) {
                $stats['geometry_seeded']++;
            }
            $parentName = avesmapsPoliticalReadParentNameFromWikiRecord($wikiRecord);
            if ($parentName !== '') {
                $parentCandidates[] = [
                    'territory_id' => (int) $territory['id'],
                    'parent_slug' => avesmapsPoliticalSlug($parentName),
                ];
            }
        }

        foreach ($parentCandidates as $candidate) {
            $parentSlug = (string) $candidate['parent_slug'];
            $childId = (int) $candidate['territory_id'];
            if ($parentSlug === '' || !isset($imported[$parentSlug])) {
                continue;
            }

            $parentId = (int) $imported[$parentSlug]['territory_id'];
            if ($parentId === $childId || !avesmapsPoliticalAssignInitialParent($pdo, $childId, $parentId)) {
                continue;
            }

            $stats['parent_assigned']++;
        }
...
function avesmapsPoliticalAssignInitialParent(PDO $pdo, int $childId, int $parentId): bool {
    $statement = $pdo->prepare(
        'UPDATE political_territory
    ...
    $relationStatement = $pdo->prepare(
        'INSERT INTO political_territory_relation (child_id, parent_id, relation_type, source, confidence)
        VALUES (:child_id, :parent_id, :relation_type, :source, :confidence)
