...
require_once dirname(__DIR__, 3) . '/political-territory-lib.php';
...
const WIKI_DOM_LOCK_FILE = dirname(__DIR__, 3) . '/wiki-dom-playground.lock';
const WIKI_DOM_MAX_ITERATIONS = 160;
const WIKI_DOM_MAX_PAGES = 100;
const WIKI_DOM_MAX_RUNTIME = 35;

$configPath = dirname(__DIR__, 3) . '/config.local.php';
if (!is_file($configPath)) jsonOut(['ok' => false, 'error' => 'config.local.php fehlt.'], 500);
$config = require $configPath;
if (!is_array($config) || !isset($config['database']) || !is_array($config['database'])) jsonOut(['ok' => false, 'error' => 'config.local.php liefert keine gÃƒÂ¼ltige database-Konfiguration.'], 500);
        ...
        while ($queue !== []) {
            ...
            try {
                if ($source !== 'entrypoint' && !isEntrypoint($title) && recordExistsByWikiUrl($pdo, pageUrl($title))) {
                    $skipped++;
                    $events[] = ['type' => 'skip', 'title' => $title, 'message' => 'Bereits in der Testtabelle vorhanden; per wiki_url ÃƒÂ¼bersprungen.'];
                    continue;
                }
...
function deleteRows(PDO $pdo, array $payload): void {
    ...
    if ($ids === []) {
        jsonOut(['ok' => false, 'error' => 'Keine gÃƒÂ¼ltigen EintrÃƒÂ¤ge ausgewÃƒÂ¤hlt.'], 400);
    }

    if (count($ids) > 500) {
        jsonOut(['ok' => false, 'error' => 'Bitte hÃƒÂ¶chstens 500 EintrÃƒÂ¤ge auf einmal lÃƒÂ¶schen.'], 400);
    }
    ...
    $result['message'] = $statement->rowCount() === 1
        ? '1 vorbereiteter Eintrag wurde gelÃƒÂ¶scht.'
        : $statement->rowCount() . ' vorbereitete EintrÃƒÂ¤ge wurden gelÃƒÂ¶scht.';

    jsonOut($result);
}

function defaultSeeds(): array { return ['Baronie/Liste', 'BergkÃƒÂ¶nigreich/Liste', 'DomÃƒÂ¤ne (Horasreich)/Liste', 'Emirat/Liste', 'Freiherrschaft/Liste', 'FÃƒÂ¼rstentum/Liste', 'Grafschaft/Liste', 'Herzogtum/Liste', 'Kaiserliches Eigengut/Liste', 'Komturei/Liste', 'KÃƒÂ¶nigreich/Liste', 'Markgrafschaft/Liste', 'Pfalzgrafschaft/Liste', 'Provinz (Imperium)/Liste', 'Provinz (Mittelreich)/Liste', 'Reichsmark/Liste', 'Republik/Liste', 'ShÃƒÂ®kanydad/Liste', 'Staat/Liste', 'Sultanat/Liste', 'Theokratie/Liste']; }
function defaultCatchwords(): array { return ['affiliation_path_labels' => ['staat', 'staatverlauf', 'zugehÃƒÂ¶rigkeit', 'zugehoerigkeit', 'politisch', 'reich', 'oberherrschaft'], 'founded_labels' => ['grÃƒÂ¼ndungsdatum', 'gruendungsdatum', 'grÃƒÂ¼ndung', 'gruendung', 'grÃƒÂ¼ndungsdaten', 'gruendungsdaten', 'unabhÃƒÂ¤ngigkeit', 'unabhaengigkeit', 'gegrÃƒÂ¼ndet', 'gegruendet', 'entstehung', 'ausrufung'], 'dissolved_labels' => ['aufgelÃƒÂ¶st', 'aufgeloest', 'auflÃƒÂ¶sung', 'aufloesung', 'ende', 'untergang', 'zerfall', 'aufgegeben'], 'period_labels' => ['zeitraum', 'bestehen', 'bestand', 'bestandszeit', 'existenz'], 'year_markers' => ['BF', 'v. BF', 'v BF'], 'founded_context_words' => ['gegrÃƒÂ¼ndet', 'gegruendet', 'grÃƒÂ¼ndung', 'gruendung', 'grÃƒÂ¼ndungsdatum', 'gruendungsdatum', 'grÃƒÂ¼ndungsdaten', 'gruendungsdaten', 'unabhÃƒÂ¤ngigkeit', 'unabhaengigkeit', 'entstand', 'entstehung', 'ausgerufen', 'ausrufung'], 'dissolved_context_words' => ['aufgelÃƒÂ¶st', 'aufgeloest', 'auflÃƒÂ¶sung', 'aufloesung', 'endete', 'untergang', 'zerfiel', 'zerfall', 'aufgegeben']]; }
...
function detectContinent(string $pageTitle, array $fields, DOMXPath $xpath): string { $context = labelKey(implode(' ', [$pageTitle, firstField($fields, ['kontinent']), firstField($fields, ['region']), firstField($fields, ['geographisch', 'geografisch']), firstField($fields, ['staat', 'staatverlauf', 'politisch']), navigationContext($xpath)])); foreach (['Myranor / GÃƒÂ¼ldenland' => ['myranor','myranors','gueldenland','gueldenlands','ras tabor','vesayama'], 'Rakshazar / Riesland' => ['rakshazar','riesland','rieslands'], 'Uthuria' => ['uthuria','uthurias'], 'Lahmaria' => ['lahmaria'], 'Aventurien' => ['aventurien','aventuriens']] as $continent => $needles) foreach ($needles as $needle) if (str_contains($context, labelKey($needle))) return $continent; return AVESMAPS_POLITICAL_DEFAULT_CONTINENT; }
...
function affiliationPath(array $fields, array $catchwords, string $fallbackTitle = ''): array { foreach ($catchwords['affiliation_path_labels'] as $label) { if (!isset($fields[$label]) || trim((string) $fields[$label]) === '') continue; $raw = trim((string) $fields[$label]); $value = preg_replace('/\b(?:geographisch|geografisch)\b\s*:?\s*[^;]+;?/iu', ' ', $raw) ?? $raw; $value = preg_replace('/\bpolitisch\b\s*:?\s*/iu', '', $value) ?? $value; $parts = preg_split('/\s*(?::|>|Ã¢â€ â€™|Ã¢â‚¬Âº|Ã‚Â»|;|,\s*(?=[A-ZÃƒâ€žÃƒâ€“ÃƒÅ“]))\s*/u', $value) ?: []; $path = []; foreach ($parts as $part) { $part = cleanText($part); if ($part !== '' && preg_match('/\b(unabhaengig|unabhÃƒÂ¤ngig|keine|unbekannt|ungeklaert|ungeklÃƒÂ¤rt|umstritten)\b/iu', $part) !== 1) $path[] = $part; } $path = array_values(array_unique($path)); if ($path !== []) return ['raw' => $raw, 'path' => $path, 'source_field' => $label]; } $self = displayName($fallbackTitle); return $self !== '' ? ['raw' => $self, 'path' => [$self], 'source_field' => 'self'] : ['raw' => '', 'path' => [], 'source_field' => '']; }
...
function periodItems(string $text, string $field): array { $parts = preg_split('/\s*(?:-|Ã¢â‚¬â€œ|Ã¢â‚¬â€|bis)\s*/u', $text) ?: []; $items = []; if (isset($parts[0]) && trim($parts[0]) !== '') $items[] = temporalItem('founded', (string) $parts[0], 'field', $field, 0.76); if (isset($parts[1]) && trim($parts[1]) !== '') $items[] = temporalItem('dissolved', (string) $parts[1], 'field', $field, 0.76); return $items; }
...
function knownTypes(): array { return ['Kaiserreich', 'SeekÃƒÂ¶nigreich', 'KÃƒÂ¶nigreich', 'BergkÃƒÂ¶nigreich', 'Imperium', 'Republik', 'Herzogtum', 'FÃƒÂ¼rstentum', 'Grafschaft', 'Baronie', 'Freiherrschaft', 'Markgrafschaft', 'Pfalzgrafschaft', 'Jarltum', 'Sultanat', 'Emirat', 'Theokratie', 'Komturei', 'DomÃƒÂ¤ne', 'Ordensland', 'Reich', 'Staat']; }
...
function labelKey(string $value): string { $value = mb_strtolower(cleanText($value), 'UTF-8'); $value = str_replace(['ÃƒÂ¤', 'ÃƒÂ¶', 'ÃƒÂ¼', 'ÃƒÅ¸'], ['ae', 'oe', 'ue', 'ss'], $value); $value = preg_replace('/[^a-z0-9]+/u', ' ', $value) ?? ''; return trim($value); }
...
function createPdo(array $db): PDO { if ((string) ($db['driver'] ?? 'mysql') !== 'mysql') throw new RuntimeException('Nur MySQL wird unterstÃƒÂ¼tzt.'); $dsn = sprintf('mysql:host=%s;port=%d;dbname=%s;charset=%s', (string) ($db['host'] ?? 'localhost'), (int) ($db['port'] ?? 3306), (string) ($db['name'] ?? ''), (string) ($db['charset'] ?? 'utf8mb4')); return new PDO($dsn, (string) ($db['user'] ?? ''), (string) ($db['password'] ?? ''), [PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION, PDO::ATTR_DEFAULT_FETCH_MODE => PDO::FETCH_ASSOC, PDO::ATTR_EMULATE_PREPARES => false]); }
...
function acquireLock() { $handle = fopen(WIKI_DOM_LOCK_FILE, 'c+'); if ($handle === false) throw new RuntimeException('Lock-Datei konnte nicht geÃƒÂ¶ffnet werden.'); if (!flock($handle, LOCK_EX | LOCK_NB)) throw new RuntimeException('Es lÃƒÂ¤uft bereits ein Wiki-DOM-Playground-Import.'); return $handle; }
