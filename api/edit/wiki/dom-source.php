...
if (!defined('AVESMAPS_WIKI_DOM_SOURCE_DIR')) {
...
require_once AVESMAPS_WIKI_DOM_SOURCE_DIR . '/political-territory-lib.php';
...
$configPath = AVESMAPS_WIKI_DOM_SOURCE_DIR . '/config.local.php';
if (!is_file($configPath)) jsonOut(['ok' => false, 'error' => 'config.local.php fehlt.'], 500);
$config = require $configPath;
if (!is_array($config) || !isset($config['database']) || !is_array($config['database'])) jsonOut(['ok' => false, 'error' => 'config.local.php liefert keine gÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¼ltige database-Konfiguration.'], 500);
        ...
        while ($queue !== []) {
            ...
            try {
                if ($source !== 'entrypoint' && !isEntrypoint($title) && recordExistsByWikiUrl($pdo, pageUrl($title))) {
                    $skipped++;
                    $events[] = ['type' => 'skip', 'title' => $title, 'message' => 'Bereits in der Testtabelle vorhanden; per wiki_url ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¼bersprungen.'];
                    continue;
                }
                ...
                foreach ($children as $section) {
                    foreach (($section['items'] ?? []) as $child) {
                        ...
                        if ($childTitle !== '') {
                            enqueue($queue, $queued, $childTitle, 'child-section:' . $resolvedTitle . ':' . (string) ($section['label'] ?? ''));
                            $childLinks++;
                        }
                    }
                }
                ...
                $stored++;
            } catch (Throwable $error) { $errors[] = ['title' => $title, 'error' => $error->getMessage()]; }
            sleepMs($options['sleep_ms']);
        }
        $result = readRows($pdo);
...
function deleteRows(PDO $pdo, array $payload): void {
    ...
    if ($ids === []) {
        jsonOut(['ok' => false, 'error' => 'Keine gÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¼ltigen EintrÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¤ge ausgewÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¤hlt.'], 400);
    }

    if (count($ids) > 500) {
        jsonOut(['ok' => false, 'error' => 'Bitte hÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¶chstens 500 EintrÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¤ge auf einmal lÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¶schen.'], 400);
    }
    ...
    $result['message'] = $statement->rowCount() === 1
        ? '1 vorbereiteter Eintrag wurde gelÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¶scht.'
        : $statement->rowCount() . ' vorbereitete EintrÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¤ge wurden gelÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¶scht.';

    jsonOut($result);
}

function defaultSeeds(): array { return ['Baronie/Liste', 'BergkÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¶nigreich/Liste', 'DomÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¤ne (Horasreich)/Liste', 'Emirat/Liste', 'Freiherrschaft/Liste', 'FÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¼rstentum/Liste', 'Grafschaft/Liste', 'Herzogtum/Liste', 'Kaiserliches Eigengut/Liste', 'Komturei/Liste', 'KÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¶nigreich/Liste', 'Markgrafschaft/Liste', 'Pfalzgrafschaft/Liste', 'Provinz (Imperium)/Liste', 'Provinz (Mittelreich)/Liste', 'Reichsmark/Liste', 'Republik/Liste', 'ShÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â®kanydad/Liste', 'Staat/Liste', 'Sultanat/Liste', 'Theokratie/Liste']; }
function defaultCatchwords(): array { return ['affiliation_path_labels' => ['staat', 'staatverlauf', 'zugehÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¶rigkeit', 'zugehoerigkeit', 'politisch', 'reich', 'oberherrschaft'], 'founded_labels' => ['grÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¼ndungsdatum', 'gruendungsdatum', 'grÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¼ndung', 'gruendung', 'grÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¼ndungsdaten', 'gruendungsdaten', 'unabhÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¤ngigkeit', 'unabhaengigkeit', 'gegrÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¼ndet', 'gegruendet', 'entstehung', 'ausrufung'], 'dissolved_labels' => ['aufgelÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¶st', 'aufgeloest', 'auflÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¶sung', 'aufloesung', 'ende', 'untergang', 'zerfall', 'aufgegeben'], 'period_labels' => ['zeitraum', 'bestehen', 'bestand', 'bestandszeit', 'existenz'], 'year_markers' => ['BF', 'v. BF', 'v BF'], 'founded_context_words' => ['gegrÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¼ndet', 'gegruendet', 'grÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¼ndung', 'gruendung', 'grÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¼ndungsdatum', 'gruendungsdatum', 'grÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¼ndungsdaten', 'gruendungsdaten', 'unabhÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¤ngigkeit', 'unabhaengigkeit', 'entstand', 'entstehung', 'ausgerufen', 'ausrufung'], 'dissolved_context_words' => ['aufgelÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¶st', 'aufgeloest', 'auflÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¶sung', 'aufloesung', 'endete', 'untergang', 'zerfiel', 'zerfall', 'aufgegeben']]; }
...
function fieldsFromHtml(DOMXPath $xpath): array { $fields = []; foreach ($xpath->query('//tr[th and td] | //tr[count(td) >= 1]') ?: [] as $row) { if (!$row instanceof DOMElement) continue; $th = $row->getElementsByTagName('th')->item(0); $tds = $row->getElementsByTagName('td'); if ($th instanceof DOMNode && $tds->item(0) instanceof DOMNode) { $key = labelKey(cellText($th, false)); $value = cellText($tds->item(0), true); } elseif ($tds->length >= 2 && $tds->item(0) instanceof DOMNode && $tds->item(1) instanceof DOMNode) { $key = labelKey(cellText($tds->item(0), false)); $value = cellText($tds->item(1), true); } elseif ($tds->length === 1 && $tds->item(0) instanceof DOMElement && (int) $tds->item(0)->getAttribute('colspan') >= 2) { $type = inferKnownType(cellText($tds->item(0), true)); if ($type !== '' && !isset($fields['typ'])) $fields['typ'] = $type; continue; } else continue; if ($key !== '' && $value !== '' && !isset($fields[$key])) $fields[$key] = $value; } return $fields; }
function cellText(DOMNode $node, bool $isValue): string { $document = new DOMDocument(); $document->appendChild($document->importNode($node->cloneNode(true), true)); $xpath = new DOMXPath($document); foreach ($xpath->query('//*[contains(translate(@style,"ABCDEFGHIJKLMNOPQRSTUVWXYZ","abcdefghijklmnopqrstuvwxyz"),"display:none") or contains(translate(@style,"ABCDEFGHIJKLMNOPQRSTUVWXYZ","abcdefghijklmnopqrstuvwxyz"),"display: none")' . ($isValue ? ' or self::small' : '') . ']') ?: [] as $remove) { if ($remove instanceof DOMNode && $remove->parentNode instanceof DOMNode) $remove->parentNode->removeChild($remove); } $text = $document->textContent ?? ''; $text = preg_replace('/\bDatierungshinweise\b/iu', ' ', $text) ?? $text; return cleanText($text); }
...
function nextElementSibling(DOMNode $node, string $tagName): ?DOMElement { $current = $node->nextSibling; while ($current instanceof DOMNode) { if ($current instanceof DOMElement && mb_strtolower($current->tagName, 'UTF-8') === mb_strtolower($tagName, 'UTF-8')) return $current; if ($current instanceof DOMElement && mb_strtolower($current->tagName, 'UTF-8') !== '') return null; $current = $current->nextSibling; } return null; }
...
function detectContinent(string $pageTitle, array $fields, DOMXPath $xpath): string { $context = labelKey(implode(' ', [$pageTitle, firstField($fields, ['kontinent']), firstField($fields, ['region']), firstField($fields, ['geographisch', 'geografisch']), firstField($fields, ['staat', 'staatverlauf', 'politisch']), navigationContext($xpath)])); foreach (['Myranor / GÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¼ldenland' => ['myranor','myranors','gueldenland','gueldenlands','ras tabor','vesayama'], 'Rakshazar / Riesland' => ['rakshazar','riesland','rieslands'], 'Uthuria' => ['uthuria','uthurias'], 'Lahmaria' => ['lahmaria'], 'Aventurien' => ['aventurien','aventuriens']] as $continent => $needles) foreach ($needles as $needle) if (str_contains($context, labelKey($needle))) return $continent; return AVESMAPS_POLITICAL_DEFAULT_CONTINENT; }
function navigationContext(DOMXPath $xpath): string { $parts = []; foreach ($xpath->query('//th[contains(.,"Staaten") or contains(.,"Reiche") or contains(.,"Herrschaftsgebiete") or contains(.,"Myranor") or contains(.,"Aventurien") or contains(.,"Rakshazar") or contains(.,"Riesland") or contains(.,"Uthuria") or contains(.,"Lahmaria")]') ?: [] as $node) $parts[] = cleanText($node instanceof DOMNode ? ($node->textContent ?? '') : ''); return implode(' ', $parts); }
function heraldryData(DOMXPath $xpath): array { $blazon = ''; $coat = ''; foreach ($xpath->query('//table[contains(@class,"infobox") or contains(@class,"Infobox") or contains(@class,"wikitable") or contains(@class,"Standard") or .//span[contains(@class,"Bildeinbindung")]]//td | //p | //li') ?: [] as $node) { if (!$node instanceof DOMNode) continue; $text = cellText($node, true); if ($blazon === '' && preg_match('/\bBlasonierung\b\s*:\s*(.+)$/iu', $text, $match) === 1) $blazon = cleanText((string) $match[1]); if ($coat === '') $coat = firstImageUrl($node); if ($blazon !== '' && $coat !== '') break; } return ['blazon' => $blazon, 'coat_of_arms_url' => $coat]; }
function firstImageUrl(DOMNode $node): string { foreach ($node instanceof DOMElement ? $node->getElementsByTagName('img') : [] as $img) { if (!$img instanceof DOMElement) continue; $src = trim((string) $img->getAttribute('src')); $largestSrc = preferLargestSrc($img, $src); $width = (int) ($img->getAttribute('width') ?: 0); $height = (int) ($img->getAttribute('height') ?: 0); $context = labelKey(implode(' ', [$src, $largestSrc, $img->getAttribute('alt'), $img->getAttribute('title'), $img->getAttribute('class'), $img->getAttribute('data-file-width'), $img->getAttribute('data-file-height')])); if ($src === '' || isRejectedHeraldryImage($context, $width, $height)) continue; if (preg_match('/\.(?:png|jpe?g|gif|webp|svg)(?:\?|$)/iu', $src) !== 1) continue; return absoluteWikiUrl($largestSrc); } return ''; }
function isRejectedHeraldryImage(string $context, int $width, int $height): bool { if ($width > 0 && $height > 0 && ($width < 60 || $height < 60)) return true; foreach (['pfeil','arrow','sort','eng'.'lish','lang'.'uage','fl'.'ag','mada'.'mal','c'.'c b'.'y','shared resources assets'] as $bad) if (str_contains($context, $bad)) return true; return false; }
function preferLargestSrc(DOMElement $img, string $src): string { $srcset = trim((string) $img->getAttribute('srcset')); if ($srcset === '') return $src; $bestUrl = $src; $bestScore = 1.0; foreach (explode(',', $srcset) as $candidate) { $parts = preg_split('/\s+/', trim($candidate)) ?: []; $url = $parts[0] ?? ''; $descriptor = $parts[1] ?? ''; if ($url === '') continue; $score = 1.0; if (preg_match('/^([0-9.]+)x$/u', $descriptor, $match) === 1) $score = (float) $match[1]; elseif (preg_match('/^(\d+)w$/u', $descriptor, $match) === 1) $score = (float) $match[1]; if ($score >= $bestScore) { $bestScore = $score; $bestUrl = $url; } } return $bestUrl; }
function absoluteWikiUrl(string $url): string { $url = trim(html_entity_decode($url, ENT_QUOTES | ENT_HTML5, 'UTF-8')); if ($url === '') return ''; if (preg_match('/^https?:\/\//iu', $url) === 1) return $url; if (str_starts_with($url, '//')) return 'https:' . $url; if (str_starts_with($url, '/')) return 'https://de.wiki-aventurica.de' . $url; return 'https://de.wiki-aventurica.de/' . ltrim($url, '/'); }
function affiliationPath(array $fields, array $catchwords, string $fallbackTitle = ''): array { foreach ($catchwords['affiliation_path_labels'] as $label) { if (!isset($fields[$label]) || trim((string) $fields[$label]) === '') continue; $raw = trim((string) $fields[$label]); $value = preg_replace('/\b(?:geographisch|geografisch)\b\s*:?\s*[^;]+;?/iu', ' ', $raw) ?? $raw; $value = preg_replace('/\bpolitisch\b\s*:?\s*/iu', '', $value) ?? $value; $parts = preg_split('/\s*(?::|>|ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢|ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Âº|ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â»|;|,\s*(?=[A-ZÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¾ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ÃƒÆ’Ã†â€™Ãƒâ€¦Ã¢â‚¬Å“]))\s*/u', $value) ?: []; $path = []; foreach ($parts as $part) { $part = cleanText($part); if ($part !== '' && preg_match('/\b(unabhaengig|unabhÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¤ngig|keine|unbekannt|ungeklaert|ungeklÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¤rt|umstritten)\b/iu', $part) !== 1) $path[] = $part; } $path = array_values(array_unique($path)); if ($path !== []) return ['raw' => $raw, 'path' => $path, 'source_field' => $label]; } $self = displayName($fallbackTitle); return $self !== '' ? ['raw' => $self, 'path' => [$self], 'source_field' => 'self'] : ['raw' => '', 'path' => [], 'source_field' => '']; }
...
function periodItems(string $text, string $field): array { $parts = preg_split('/\s*(?:-|ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œ|ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â|bis)\s*/u', $text) ?: []; $items = []; if (isset($parts[0]) && trim($parts[0]) !== '') $items[] = temporalItem('founded', (string) $parts[0], 'field', $field, 0.76); if (isset($parts[1]) && trim($parts[1]) !== '') $items[] = temporalItem('dissolved', (string) $parts[1], 'field', $field, 0.76); return $items; }
...
function buildRecord(string $pageTitle, array $fields, array $pathInfo, array $temporal, array $children, string $continent, array $heraldry): array { $founded = bestTemporal($temporal, 'founded'); $dissolved = bestTemporal($temporal, 'dissolved'); $path = is_array($pathInfo['path'] ?? null) ? $pathInfo['path'] : []; $url = pageUrl($pageTitle); $record = ['Name' => displayName($pageTitle), 'Typ' => inferType($pageTitle, firstField($fields, ['typ', 'art', 'herrschaftsgebiet'])), 'Kontinent' => $continent, 'Zugehoerigkeit' => (string) ($pathInfo['raw'] ?? ''), 'Zugehoerigkeit-Key' => $path !== [] ? avesmapsPoliticalSlug($path[0]) : '', 'Zugehoerigkeit-Root' => $path[0] ?? '', 'Zugehoerigkeit-Pfad' => implode(' > ', $path), 'Zugehoerigkeit-JSON' => ['path' => $path, 'source' => 'wiki-dom-playground-seed', 'source_field' => (string) ($pathInfo['source_field'] ?? '')], 'Status' => firstField($fields, ['status']), 'Herrschaftsform' => firstField($fields, ['herrschaftsform']), 'Hauptstadt' => firstField($fields, ['hauptstadt']), 'Herrschaftssitz' => firstField($fields, ['herrschaftssitz']), 'Oberhaupt' => firstField($fields, ['oberhaupt']), 'Sprache' => firstField($fields, ['sprache']), 'Waehrung' => firstField($fields, ['waehrung', 'wahrung']), 'Handelswaren' => firstField($fields, ['handelswaren']), 'Einwohnerzahl' => firstField($fields, ['einwohnerzahl', 'einwohner']), 'Gruender' => firstField($fields, ['gruender']), 'Geographisch' => firstField($fields, ['geographisch', 'geografisch', 'region']), 'Politisch' => firstField($fields, ['politisch', 'staat']), 'Handelszone' => firstField($fields, ['handelszone']), 'Blasonierung' => firstField($fields, ['blasonierung']) ?: (string) ($heraldry['blazon'] ?? ''), 'Wappen-Link' => (string) ($heraldry['coat_of_arms_url'] ?? ''), 'Gruendungsdatum-Text' => (string) ($founded['raw_text'] ?? firstField($fields, ['gruendungsdatum', 'gruendung', 'unabhaengigkeit'])), 'Gruendungsdatum-StartBF' => $founded['start_bf'] ?? null, 'Gruendungsdatum-EndBF' => $founded['end_bf'] ?? null, 'Gruendungsdatum-JSON' => $founded ?: [], 'Aufgeloest-Text' => (string) ($dissolved['raw_text'] ?? firstField($fields, ['aufgeloest', 'aufloesung'])), 'Aufgeloest-StartBF' => $dissolved['start_bf'] ?? null, 'Aufgeloest-EndBF' => $dissolved['end_bf'] ?? null, 'Aufgeloest-JSON' => $dissolved ?: [], 'Wiki-Link' => $url, 'raw_json' => ['source' => 'wiki-dom-playground-seed', 'fields' => $fields, 'path' => $pathInfo, 'temporal' => $temporal, 'children' => $children, 'continent' => $continent, 'heraldry' => $heraldry]]; return avesmapsPoliticalNormalizeWikiRecord($record); }
function firstField(array $fields, array $keys): string { foreach ($keys as $key) { if (isset($fields[$key]) && trim((string) $fields[$key]) !== '') return trim((string) $fields[$key]); } return ''; }
...
function inferKnownType(string $text): string { foreach (knownTypes() as $type) if (preg_match('/\b' . preg_quote($type, '/') . '\b/iu', $text) === 1) return $type; return ''; }
function inferType(string $pageTitle, string $fallback): string { $type = trim($fallback) !== '' ? inferKnownType($fallback) : ''; if ($type !== '') return $type; return inferKnownType($pageTitle); }
function knownTypes(): array { return ['Kaiserreich', 'SeekÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¶nigreich', 'KÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¶nigreich', 'BergkÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¶nigreich', 'Imperium', 'Republik', 'Herzogtum', 'FÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¼rstentum', 'Grafschaft', 'Baronie', 'Freiherrschaft', 'Markgrafschaft', 'Pfalzgrafschaft', 'Jarltum', 'Sultanat', 'Emirat', 'Theokratie', 'Komturei', 'DomÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¤ne', 'Ordensland', 'Reich', 'Staat']; }
...
function labelKey(string $value): string { $value = mb_strtolower(cleanText($value), 'UTF-8'); $value = str_replace(['ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¤', 'ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¶', 'ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¼', 'ÃƒÆ’Ã†â€™Ãƒâ€¦Ã‚Â¸'], ['ae', 'oe', 'ue', 'ss'], $value); $value = preg_replace('/[^a-z0-9]+/u', ' ', $value) ?? ''; return trim($value); }
...
function createPdo(array $db): PDO { if ((string) ($db['driver'] ?? 'mysql') !== 'mysql') throw new RuntimeException('Nur MySQL wird unterstÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¼tzt.'); $dsn = sprintf('mysql:host=%s;port=%d;dbname=%s;charset=%s', (string) ($db['host'] ?? 'localhost'), (int) ($db['port'] ?? 3306), (string) ($db['name'] ?? ''), (string) ($db['charset'] ?? 'utf8mb4')); return new PDO($dsn, (string) ($db['user'] ?? ''), (string) ($db['password'] ?? ''), [PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION, PDO::ATTR_DEFAULT_FETCH_MODE => PDO::FETCH_ASSOC, PDO::ATTR_EMULATE_PREPARES => false]); }
...
function acquireLock() { $handle = fopen(WIKI_DOM_LOCK_FILE, 'c+'); if ($handle === false) throw new RuntimeException('Lock-Datei konnte nicht geÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¶ffnet werden.'); if (!flock($handle, LOCK_EX | LOCK_NB)) throw new RuntimeException('Es lÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¤uft bereits ein Wiki-DOM-Playground-Import.'); return $handle; }
