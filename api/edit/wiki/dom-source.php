...
require_once dirname(__DIR__, 2) . '/political-territory-lib.php';
...
const WIKI_DOM_LOCK_FILE = dirname(__DIR__, 2) . '/wiki-dom-playground.lock';
...
if (!is_array($config) || !isset($config['database']) || !is_array($config['database'])) jsonOut(['ok' => false, 'error' => 'config.local.php liefert keine gÃ¼ltige database-Konfiguration.'], 500);
        ...
        while ($queue !== []) {
            ...
            try {
                if ($source !== 'entrypoint' && !isEntrypoint($title) && recordExistsByWikiUrl($pdo, pageUrl($title))) {
                    $skipped++;
                    $events[] = ['type' => 'skip', 'title' => $title, 'message' => 'Bereits in der Testtabelle vorhanden; per wiki_url Ã¼bersprungen.'];
                    continue;
                }
...
function deleteRows(PDO $pdo, array $payload): void {
    ...
    if ($ids === []) {
        jsonOut(['ok' => false, 'error' => 'Keine gÃ¼ltigen EintrÃ¤ge ausgewÃ¤hlt.'], 400);
    }

    if (count($ids) > 500) {
        jsonOut(['ok' => false, 'error' => 'Bitte hÃ¶chstens 500 EintrÃ¤ge auf einmal lÃ¶schen.'], 400);
    }
    ...
    $result['message'] = $statement->rowCount() === 1
        ? '1 vorbereiteter Eintrag wurde gelÃ¶scht.'
        : $statement->rowCount() . ' vorbereitete EintrÃ¤ge wurden gelÃ¶scht.';

    jsonOut($result);
}

function defaultSeeds(): array { return ['Baronie/Liste', 'BergkÃ¶nigreich/Liste', 'DomÃ¤ne (Horasreich)/Liste', 'Emirat/Liste', 'Freiherrschaft/Liste', 'FÃ¼rstentum/Liste', 'Grafschaft/Liste', 'Herzogtum/Liste', 'Kaiserliches Eigengut/Liste', 'Komturei/Liste', 'KÃ¶nigreich/Liste', 'Markgrafschaft/Liste', 'Pfalzgrafschaft/Liste', 'Provinz (Imperium)/Liste', 'Provinz (Mittelreich)/Liste', 'Reichsmark/Liste', 'Republik/Liste', 'ShÃ®kanydad/Liste', 'Staat/Liste', 'Sultanat/Liste', 'Theokratie/Liste']; }
function defaultCatchwords(): array { return ['affiliation_path_labels' => ['staat', 'staatverlauf', 'zugehÃ¶rigkeit', 'zugehoerigkeit', 'politisch', 'reich', 'oberherrschaft'], 'founded_labels' => ['grÃ¼ndungsdatum', 'gruendungsdatum', 'grÃ¼ndung', 'gruendung', 'grÃ¼ndungsdaten', 'gruendungsdaten', 'unabhÃ¤ngigkeit', 'unabhaengigkeit', 'gegrÃ¼ndet', 'gegruendet', 'entstehung', 'ausrufung'], 'dissolved_labels' => ['aufgelÃ¶st', 'aufgeloest', 'auflÃ¶sung', 'aufloesung', 'ende', 'untergang', 'zerfall', 'aufgegeben'], 'period_labels' => ['zeitraum', 'bestehen', 'bestand', 'bestandszeit', 'existenz'], 'year_markers' => ['BF', 'v. BF', 'v BF'], 'founded_context_words' => ['gegrÃ¼ndet', 'gegruendet', 'grÃ¼ndung', 'gruendung', 'grÃ¼ndungsdatum', 'gruendungsdatum', 'grÃ¼ndungsdaten', 'gruendungsdaten', 'unabhÃ¤ngigkeit', 'unabhaengigkeit', 'entstand', 'entstehung', 'ausgerufen', 'ausrufung'], 'dissolved_context_words' => ['aufgelÃ¶st', 'aufgeloest', 'auflÃ¶sung', 'aufloesung', 'endete', 'untergang', 'zerfiel', 'zerfall', 'aufgegeben']]; }
...
function detectContinent(string $pageTitle, array $fields, DOMXPath $xpath): string { $context = labelKey(implode(' ', [$pageTitle, firstField($fields, ['kontinent']), firstField($fields, ['region']), firstField($fields, ['geographisch', 'geografisch']), firstField($fields, ['staat', 'staatverlauf', 'politisch']), navigationContext($xpath)])); foreach (['Myranor / GÃ¼ldenland' => ['myranor','myranors','gueldenland','gueldenlands','ras tabor','vesayama'], 'Rakshazar / Riesland' => ['rakshazar','riesland','rieslands'], 'Uthuria' => ['uthuria','uthurias'], 'Lahmaria' => ['lahmaria'], 'Aventurien' => ['aventurien','aventuriens']] as $continent => $needles) foreach ($needles as $needle) if (str_contains($context, labelKey($needle))) return $continent; return AVESMAPS_POLITICAL_DEFAULT_CONTINENT; }
...
function affiliationPath(array $fields, array $catchwords, string $fallbackTitle = ''): array { foreach ($catchwords['affiliation_path_labels'] as $label) { if (!isset($fields[$label]) || trim((string) $fields[$label]) === '') continue; $raw = trim((string) $fields[$label]); $value = preg_replace('/\b(?:geographisch|geografisch)\b\s*:?\s*[^;]+;?/iu', ' ', $raw) ?? $raw; $value = preg_replace('/\bpolitisch\b\s*:?\s*/iu', '', $value) ?? $value; $parts = preg_split('/\s*(?::|>|â†’|â€º|Â»|;|,\s*(?=[A-ZÃ„Ã–Ãœ]))\s*/u', $value) ?: []; $path = []; foreach ($parts as $part) { $part = cleanText($part); if ($part !== '' && preg_match('/\b(unabhaengig|unabhÃ¤ngig|keine|unbekannt|ungeklaert|ungeklÃ¤rt|umstritten)\b/iu', $part) !== 1) $path[] = $part; } $path = array_values(array_unique($path)); if ($path !== []) return ['raw' => $raw, 'path' => $path, 'source_field' => $label]; } $self = displayName($fallbackTitle); return $self !== '' ? ['raw' => $self, 'path' => [$self], 'source_field' => 'self'] : ['raw' => '', 'path' => [], 'source_field' => '']; }
...
function periodItems(string $text, string $field): array { $parts = preg_split('/\s*(?:-|â€“|â€”|bis)\s*/u', $text) ?: []; $items = []; if (isset($parts[0]) && trim($parts[0]) !== '') $items[] = temporalItem('founded', (string) $parts[0], 'field', $field, 0.76); if (isset($parts[1]) && trim($parts[1]) !== '') $items[] = temporalItem('dissolved', (string) $parts[1], 'field', $field, 0.76); return $items; }
...
function knownTypes(): array { return ['Kaiserreich', 'SeekÃ¶nigreich', 'KÃ¶nigreich', 'BergkÃ¶nigreich', 'Imperium', 'Republik', 'Herzogtum', 'FÃ¼rstentum', 'Grafschaft', 'Baronie', 'Freiherrschaft', 'Markgrafschaft', 'Pfalzgrafschaft', 'Jarltum', 'Sultanat', 'Emirat', 'Theokratie', 'Komturei', 'DomÃ¤ne', 'Ordensland', 'Reich', 'Staat']; }
...
function labelKey(string $value): string { $value = mb_strtolower(cleanText($value), 'UTF-8'); $value = str_replace(['Ã¤', 'Ã¶', 'Ã¼', 'ÃŸ'], ['ae', 'oe', 'ue', 'ss'], $value); $value = preg_replace('/[^a-z0-9]+/u', ' ', $value) ?? ''; return trim($value); }
...
function createPdo(array $db): PDO { if ((string) ($db['driver'] ?? 'mysql') !== 'mysql') throw new RuntimeException('Nur MySQL wird unterstÃ¼tzt.'); $dsn = sprintf('mysql:host=%s;port=%d;dbname=%s;charset=%s', (string) ($db['host'] ?? 'localhost'), (int) ($db['port'] ?? 3306), (string) ($db['name'] ?? ''), (string) ($db['charset'] ?? 'utf8mb4')); return new PDO($dsn, (string) ($db['user'] ?? ''), (string) ($db['password'] ?? ''), [PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION, PDO::ATTR_DEFAULT_FETCH_MODE => PDO::FETCH_ASSOC, PDO::ATTR_EMULATE_PREPARES => false]); }
...
function acquireLock() { $handle = fopen(WIKI_DOM_LOCK_FILE, 'c+'); if ($handle === false) throw new RuntimeException('Lock-Datei konnte nicht geÃ¶ffnet werden.'); if (!flock($handle, LOCK_EX | LOCK_NB)) throw new RuntimeException('Es lÃ¤uft bereits ein Wiki-DOM-Playground-Import.'); return $handle; }
