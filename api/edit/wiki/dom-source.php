...
if (!is_array($config) || !isset($config['database']) || !is_array($config['database'])) jsonOut(['ok' => false, 'error' => 'config.local.php liefert keine gÃƒÆ’Ã‚Â¼ltige database-Konfiguration.'], 500);
        ...
        while ($queue !== []) {
            ...
            try {
                if ($source !== 'entrypoint' && !isEntrypoint($title) && recordExistsByWikiUrl($pdo, pageUrl($title))) {
                    $skipped++;
                    $events[] = ['type' => 'skip', 'title' => $title, 'message' => 'Bereits in der Testtabelle vorhanden; per wiki_url ÃƒÆ’Ã‚Â¼bersprungen.'];
                    continue;
                }
...
function deleteRows(PDO $pdo, array $payload): void {
    ...
    if ($ids === []) {
        jsonOut(['ok' => false, 'error' => 'Keine gÃƒÆ’Ã‚Â¼ltigen EintrÃƒÆ’Ã‚Â¤ge ausgewÃƒÆ’Ã‚Â¤hlt.'], 400);
    }

    if (count($ids) > 500) {
        jsonOut(['ok' => false, 'error' => 'Bitte hÃƒÆ’Ã‚Â¶chstens 500 EintrÃƒÆ’Ã‚Â¤ge auf einmal lÃƒÆ’Ã‚Â¶schen.'], 400);
    }
    ...
    $result['message'] = $statement->rowCount() === 1
        ? '1 vorbereiteter Eintrag wurde gelÃƒÆ’Ã‚Â¶scht.'
        : $statement->rowCount() . ' vorbereitete EintrÃƒÆ’Ã‚Â¤ge wurden gelÃƒÆ’Ã‚Â¶scht.';

    jsonOut($result);
}

function defaultSeeds(): array { return ['Baronie/Liste', 'BergkÃƒÆ’Ã‚Â¶nigreich/Liste', 'DomÃƒÆ’Ã‚Â¤ne (Horasreich)/Liste', 'Emirat/Liste', 'Freiherrschaft/Liste', 'FÃƒÆ’Ã‚Â¼rstentum/Liste', 'Grafschaft/Liste', 'Herzogtum/Liste', 'Kaiserliches Eigengut/Liste', 'Komturei/Liste', 'KÃƒÆ’Ã‚Â¶nigreich/Liste', 'Markgrafschaft/Liste', 'Pfalzgrafschaft/Liste', 'Provinz (Imperium)/Liste', 'Provinz (Mittelreich)/Liste', 'Reichsmark/Liste', 'Republik/Liste', 'ShÃƒÆ’Ã‚Â®kanydad/Liste', 'Staat/Liste', 'Sultanat/Liste', 'Theokratie/Liste']; }
function defaultCatchwords(): array { return ['affiliation_path_labels' => ['staat', 'staatverlauf', 'zugehÃƒÆ’Ã‚Â¶rigkeit', 'zugehoerigkeit', 'politisch', 'reich', 'oberherrschaft'], 'founded_labels' => ['grÃƒÆ’Ã‚Â¼ndungsdatum', 'gruendungsdatum', 'grÃƒÆ’Ã‚Â¼ndung', 'gruendung', 'grÃƒÆ’Ã‚Â¼ndungsdaten', 'gruendungsdaten', 'unabhÃƒÆ’Ã‚Â¤ngigkeit', 'unabhaengigkeit', 'gegrÃƒÆ’Ã‚Â¼ndet', 'gegruendet', 'entstehung', 'ausrufung'], 'dissolved_labels' => ['aufgelÃƒÆ’Ã‚Â¶st', 'aufgeloest', 'auflÃƒÆ’Ã‚Â¶sung', 'aufloesung', 'ende', 'untergang', 'zerfall', 'aufgegeben'], 'period_labels' => ['zeitraum', 'bestehen', 'bestand', 'bestandszeit', 'existenz'], 'year_markers' => ['BF', 'v. BF', 'v BF'], 'founded_context_words' => ['gegrÃƒÆ’Ã‚Â¼ndet', 'gegruendet', 'grÃƒÆ’Ã‚Â¼ndung', 'gruendung', 'grÃƒÆ’Ã‚Â¼ndungsdatum', 'gruendungsdatum', 'grÃƒÆ’Ã‚Â¼ndungsdaten', 'gruendungsdaten', 'unabhÃƒÆ’Ã‚Â¤ngigkeit', 'unabhaengigkeit', 'entstand', 'entstehung', 'ausgerufen', 'ausrufung'], 'dissolved_context_words' => ['aufgelÃƒÆ’Ã‚Â¶st', 'aufgeloest', 'auflÃƒÆ’Ã‚Â¶sung', 'aufloesung', 'endete', 'untergang', 'zerfiel', 'zerfall', 'aufgegeben']]; }
...
function detectContinent(string $pageTitle, array $fields, DOMXPath $xpath): string { $context = labelKey(implode(' ', [$pageTitle, firstField($fields, ['kontinent']), firstField($fields, ['region']), firstField($fields, ['geographisch', 'geografisch']), firstField($fields, ['staat', 'staatverlauf', 'politisch']), navigationContext($xpath)])); foreach (['Myranor / GÃƒÆ’Ã‚Â¼ldenland' => ['myranor','myranors','gueldenland','gueldenlands','ras tabor','vesayama'], 'Rakshazar / Riesland' => ['rakshazar','riesland','rieslands'], 'Uthuria' => ['uthuria','uthurias'], 'Lahmaria' => ['lahmaria'], 'Aventurien' => ['aventurien','aventuriens']] as $continent => $needles) foreach ($needles as $needle) if (str_contains($context, labelKey($needle))) return $continent; return AVESMAPS_POLITICAL_DEFAULT_CONTINENT; }
...
function affiliationPath(array $fields, array $catchwords, string $fallbackTitle = ''): array { foreach ($catchwords['affiliation_path_labels'] as $label) { if (!isset($fields[$label]) || trim((string) $fields[$label]) === '') continue; $raw = trim((string) $fields[$label]); $value = preg_replace('/\b(?:geographisch|geografisch)\b\s*:?\s*[^;]+;?/iu', ' ', $raw) ?? $raw; $value = preg_replace('/\bpolitisch\b\s*:?\s*/iu', '', $value) ?? $value; $parts = preg_split('/\s*(?::|>|ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢|ÃƒÂ¢Ã¢â€šÂ¬Ã‚Âº|Ãƒâ€šÃ‚Â»|;|,\s*(?=[A-ZÃƒÆ’Ã¢â‚¬Å¾ÃƒÆ’Ã¢â‚¬â€œÃƒÆ’Ã…â€œ]))\s*/u', $value) ?: []; $path = []; foreach ($parts as $part) { $part = cleanText($part); if ($part !== '' && preg_match('/\b(unabhaengig|unabhÃƒÆ’Ã‚Â¤ngig|keine|unbekannt|ungeklaert|ungeklÃƒÆ’Ã‚Â¤rt|umstritten)\b/iu', $part) !== 1) $path[] = $part; } $path = array_values(array_unique($path)); if ($path !== []) return ['raw' => $raw, 'path' => $path, 'source_field' => $label]; } $self = displayName($fallbackTitle); return $self !== '' ? ['raw' => $self, 'path' => [$self], 'source_field' => 'self'] : ['raw' => '', 'path' => [], 'source_field' => '']; }
...
function knownTypes(): array { return ['Kaiserreich', 'SeekÃƒÆ’Ã‚Â¶nigreich', 'KÃƒÆ’Ã‚Â¶nigreich', 'BergkÃƒÆ’Ã‚Â¶nigreich', 'Imperium', 'Republik', 'Herzogtum', 'FÃƒÆ’Ã‚Â¼rstentum', 'Grafschaft', 'Baronie', 'Freiherrschaft', 'Markgrafschaft', 'Pfalzgrafschaft', 'Jarltum', 'Sultanat', 'Emirat', 'Theokratie', 'Komturei', 'DomÃƒÆ’Ã‚Â¤ne', 'Ordensland', 'Reich', 'Staat']; }
...
function labelKey(string $value): string { $value = mb_strtolower(cleanText($value), 'UTF-8'); $value = str_replace(['ÃƒÆ’Ã‚Â¤', 'ÃƒÆ’Ã‚Â¶', 'ÃƒÆ’Ã‚Â¼', 'ÃƒÆ’Ã…Â¸'], ['ae', 'oe', 'ue', 'ss'], $value); $value = preg_replace('/[^a-z0-9]+/u', ' ', $value) ?? ''; return trim($value); }
...
function createPdo(array $db): PDO { if ((string) ($db['driver'] ?? 'mysql') !== 'mysql') throw new RuntimeException('Nur MySQL wird unterstÃƒÆ’Ã‚Â¼tzt.'); $dsn = sprintf('mysql:host=%s;port=%d;dbname=%s;charset=%s', (string) ($db['host'] ?? 'localhost'), (int) ($db['port'] ?? 3306), (string) ($db['name'] ?? ''), (string) ($db['charset'] ?? 'utf8mb4')); return new PDO($dsn, (string) ($db['user'] ?? ''), (string) ($db['password'] ?? ''), [PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION, PDO::ATTR_DEFAULT_FETCH_MODE => PDO::FETCH_ASSOC, PDO::ATTR_EMULATE_PREPARES => false]); }
...
function acquireLock() { $handle = fopen(WIKI_DOM_LOCK_FILE, 'c+'); if ($handle === false) throw new RuntimeException('Lock-Datei konnte nicht geÃƒÆ’Ã‚Â¶ffnet werden.'); if (!flock($handle, LOCK_EX | LOCK_NB)) throw new RuntimeException('Es lÃƒÆ’Ã‚Â¤uft bereits ein Wiki-DOM-Playground-Import.'); return $handle; }
