...
// Authed game-literature cover endpoint (cap 'edit'). Two form-POST modes, both keyed by public_id:
...
try {
    ...
    if ($mode === 'refetch' && !$hasFile) {
        ...
        if ($wikiKey === '') {
            avesmapsErrorResponse(400, 'invalid_request', 'Dieser Literatur-Eintrag hat keinen Wiki-Eintrag — es gibt kein Wiki-Cover zum Ziehen.');
        }
        ...
        if ($coverFile === '') {
            avesmapsErrorResponse(404, 'not_found', 'Kein Wiki-Cover im Staging. Zuerst „Dump holen" + „Literatur syncen".');
        }
    ...
    if ($originalBytes !== '') {
        $scaledBytes = avesmapsWikiSyncMonitorDownscaleCoatBytes($originalBytes, $ext, AVESMAPS_GAME_LITERATURE_COVER_MAX_EDGE);
