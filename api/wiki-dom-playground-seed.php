    ...
    try {
        ...
        while ($queue !== []) {
            ...
            try {
                ...
                upsertRecord($pdo, buildRecord($resolvedTitle, $fields, $path, $temporal, $children, $continent));
                $stored++;
...
function fieldsFromHtml(DOMXPath $xpath): array { $fields = []; foreach ($xpath->query('//tr[th and td] | //tr[count(td) >= 2]') ?: [] as $row) { if (!$row instanceof DOMElement) continue; $th = $row->getElementsByTagName('th')->item(0); $tds = $row->getElementsByTagName('td'); if ($th instanceof DOMNode && $tds->item(0) instanceof DOMNode) { $key = labelKey(cellText($th, false)); $value = cellText($tds->item(0), true); } elseif ($tds->length >= 2 && $tds->item(0) instanceof DOMNode && $tds->item(1) instanceof DOMNode) { $key = labelKey(cellText($tds->item(0), false)); $value = cellText($tds->item(1), true); } elseif ($tds->length === 1 && $tds->item(0) instanceof DOMElement && (int) $tds->item(0)->getAttribute('colspan') >= 2) { $type = inferType('', cellText($tds->item(0), true)); if ($type !== '' && !isset($fields['typ'])) $fields['typ'] = $type; continue; } else continue; if ($key !== '' && $value !== '' && !isset($fields[$key])) $fields[$key] = $value; } return $fields; }
...
function inferType(string $pageTitle, string $fallback): string { if (trim($fallback) !== '') { foreach (knownTypes() as $type) if (preg_match('/\b' . preg_quote($type, '/') . '\b/iu', $fallback) === 1) return $type; return trim($fallback); } foreach (knownTypes() as $type) if (preg_match('/\b' . preg_quote($type, '/') . '\b/iu', $pageTitle) === 1) return $type; return ''; }
function knownTypes(): array { return ['Kaiserreich', 'Königreich', 'Bergkönigreich', 'Imperium', 'Republik', 'Herzogtum', 'Fürstentum', 'Grafschaft', 'Baronie', 'Freiherrschaft', 'Markgrafschaft', 'Pfalzgrafschaft', 'Jarltum', 'Sultanat', 'Emirat', 'Theokratie', 'Komturei', 'Domäne', 'Ordensland', 'Reich', 'Staat']; }
