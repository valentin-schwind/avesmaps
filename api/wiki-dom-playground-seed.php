...
function fetchHtml(string $pageTitle, int $timeout): string {
    ...
    if (function_exists('curl_init')) {
        ...
        if ($curl !== false) {
            ...
            curl_close($curl);
            if (is_string($html) && trim($html) !== '' && $status >= 200 && $status < 400) return $html;
...
function affiliationPath(array $fields, array $catchwords): array {
    foreach ($catchwords['affiliation_path_labels'] as $label) {
        ...
        $path = [];
        foreach ($parts as $part) { $part = cleanText($part); if ($part !== '' && preg_match('/\b(unabhaengig|unabhängig|keine|unbekannt|ungeklaert|ungeklärt)\b/iu', $part) !== 1) $path[] = $part; }
