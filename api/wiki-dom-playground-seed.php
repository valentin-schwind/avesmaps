...
function fieldsFromHtml(DOMXPath $xpath): array {
    ...
    foreach ($xpath->query('//tr[th and td] | //tr[count(td) >= 2]') ?: [] as $row) {
        ...
        if ($th instanceof DOMNode && $tds->item(0) instanceof DOMNode) { $key = labelKey(cellText($th, false)); $value = cellText($tds->item(0), true); }
        elseif ($tds->length >= 2 && $tds->item(0) instanceof DOMNode && $tds->item(1) instanceof DOMNode) { $key = labelKey(cellText($tds->item(0), false)); $value = cellText($tds->item(1), true); }
        else continue;
...
function upsertRecord(PDO $pdo, array $record): void {
    $columns = tableColumns($pdo);
    $write = [];
    foreach ($record as $key => $value) {
        if (isset($columns[$key]) && !in_array($key, ['id', 'synced_at'], true)) $write[$key] = $value;
    }
    if (!isset($write['wiki_key'], $write['name'])) throw new RuntimeException('Datensatz ohne wiki_key/name kann nicht gespeichert werden.');
    $names = array_keys($write);
    $insertCols = implode(', ', $names);
    $insertValues = ':' . implode(', :', $names);
    $updates = implode(', ', array_map(static fn(string $col): string => $col . '=VALUES(' . $col . ')', array_filter($names, static fn(string $col): bool => $col !== 'wiki_key')));
    $sql = 'INSERT INTO ' . WIKI_DOM_TEST_TABLE . ' (' . $insertCols . ', synced_at) VALUES (' . $insertValues . ', CURRENT_TIMESTAMP(3)) ON DUPLICATE KEY UPDATE ' . $updates . ', synced_at=CURRENT_TIMESTAMP(3)';
    $params = [];
    foreach ($write as $key => $value) {
        if (is_array($value)) $params[$key] = jsonOrNull($value);
        elseif (is_string($value)) $params[$key] = nullable($value);
