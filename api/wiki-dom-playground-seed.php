    ...
    try {
        ...
        while ($queue !== []) {
            ...
            try {
                ...
                upsertRecord($pdo, buildRecord($resolvedTitle, $fields, $path, $temporal, $children));
                $stored++;
            } catch (Throwable $error) {
        ...
        $result = readRows($pdo);
        $result['run'] = ['ok' => true, 'runtime_seconds' => round(microtime(true) - $startedAt, 3), 'iterations' => $iterations, 'entrypoint_pages' => $entrypoints, 'fetched_pages' => $stored, 'skipped_existing' => $skipped, 'queued_child_links' => $childLinks, 'queued_remaining' => count($queue), 'relations' => $childLinks, 'events' => $events, 'errors' => $errors, 'options' => $options];
        $result['relations'] = [];
        jsonOut($result);
...
function fieldsFromHtml(DOMXPath $xpath): array { $fields = []; foreach ($xpath->query('//tr[th and td] | //tr[count(td) >= 2]') ?: [] as $row) { if (!$row instanceof DOMElement) continue; $th = $row->getElementsByTagName('th')->item(0); $tds = $row->getElementsByTagName('td'); if ($th instanceof DOMNode && $tds->item(0) instanceof DOMNode) { $key = labelKey(cellText($th, false)); $value = cellText($tds->item(0), true); } elseif ($tds->length >= 2 && $tds->item(0) instanceof DOMNode && $tds->item(1) instanceof DOMNode) { $key = labelKey(cellText($tds->item(0), false)); $value = cellText($tds->item(1), true); } else continue; if ($key !== '' && $value !== '' && !isset($fields[$key])) $fields[$key] = $value; } return $fields; }
