...
if (!is_file($configPath)) {
    jsonOut(['ok' => false, 'error' => 'config.local.php fehlt.'], 500);
}
$config = require $configPath;
if (!is_array($config) || !isset($config['database']) || !is_array($config['database'])) {
    jsonOut(['ok' => false, 'error' => 'config.local.php liefert keine gültige database-Konfiguration.'], 500);
}
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        $payload = readBody();
        if ($action === 'run') {
            runImport($pdo, $payload);
        }
        if ($action === 'clear') {
            $pdo->exec('TRUNCATE TABLE ' . WIKI_DOM_TEST_TABLE);
            jsonOut(['ok' => true, 'message' => 'Test-Tabelle geleert.']);
        }
        jsonOut(['ok' => false, 'error' => 'Unbekannte POST-Action.'], 400);
    }

    if ($_SERVER['REQUEST_METHOD'] !== 'GET') {
        jsonOut(['ok' => false, 'error' => 'Nur GET und POST sind erlaubt.'], 405);
    }
    if ($action === 'defaults') {
        jsonOut(['ok' => true, 'defaults' => defaultOptions(), 'seeds' => defaultSeeds(), 'catchwords' => defaultCatchwords()]);
    }
    if ($action === 'list') {
        jsonOut(readRows($pdo));
    }
...
function runImport(PDO $pdo, array $payload): void {
    ...
    try {
        ...
        $queue = [];
        $queued = [];
        foreach (seedInputs($payload['seeds'] ?? defaultSeeds()) as $title) {
            enqueue($queue, $queued, $title, 'entrypoint');
        }
        if ($queue === []) {
        ...
        while ($queue !== []) {
            ...
            try {
                $xpath = xpath(fetchHtml($title, $options['request_timeout_seconds']));
                ...
                if ($source === 'entrypoint' || isEntrypoint($title) || isEntrypoint($resolvedTitle)) {
                    ...
                    foreach ($links as $linkTitle) {
                        enqueue($queue, $queued, $linkTitle, 'seed-list:' . $resolvedTitle);
                    }
        ...
        $result = readRows($pdo);
        $result['run'] = ['ok' => true, 'runtime_seconds' => round(microtime(true) - $startedAt, 3), 'iterations' => $iterations, 'entrypoint_pages' => $entrypoints, 'fetched_pages' => $stored, 'queued_remaining' => count($queue), 'relations' => 0, 'events' => $events, 'errors' => $errors, 'options' => $options];
        $result['relations'] = [];
        jsonOut($result);
    } finally {
        releaseLock($lock);
...
function defaultCatchwords(): array {
    return [
        'affiliation_path_labels' => ['zugehörigkeit', 'zugehoerigkeit', 'politisch', 'staat', 'reich', 'oberherrschaft'],
        'founded_labels' => ['gründung', 'gruendung', 'gründungsdatum', 'gruendungsdatum', 'gründungsdaten', 'gruendungsdaten', 'gegründet', 'gegruendet', 'entstehung', 'ausrufung'],
        'dissolved_labels' => ['auflösung', 'aufloesung', 'aufgelöst', 'aufgeloest', 'ende', 'untergang', 'zerfall', 'aufgegeben'],
        'period_labels' => ['zeitraum', 'bestehen', 'bestand', 'bestandszeit', 'existenz'],
        'year_markers' => ['BF', 'v. BF', 'v BF'],
        'founded_context_words' => ['gegründet', 'gegruendet', 'gründung', 'gruendung', 'gründungsdatum', 'gruendungsdatum', 'gründungsdaten', 'gruendungsdaten', 'entstand', 'entstehung', 'ausgerufen', 'ausrufung'],
...
function defaultOptions(): array {
    return ['max_iterations' => 30, 'max_pages' => 20, 'max_runtime_seconds' => 20, 'sleep_ms' => 450, 'request_timeout_seconds' => 8];
}

function options(array $payload): array {
    $defaults = defaultOptions();
    return ['max_iterations' => max(1, min(WIKI_DOM_MAX_ITERATIONS, (int) ($payload['max_iterations'] ?? $defaults['max_iterations']))), 'max_pages' => max(1, min(WIKI_DOM_MAX_PAGES, (int) ($payload['max_pages'] ?? $defaults['max_pages']))), 'max_runtime_seconds' => max(3, min(WIKI_DOM_MAX_RUNTIME, (int) ($payload['max_runtime_seconds'] ?? $defaults['max_runtime_seconds']))), 'sleep_ms' => max(0, min(5000, (int) ($payload['sleep_ms'] ?? $defaults['sleep_ms']))), 'request_timeout_seconds' => max(3, min(20, (int) ($payload['request_timeout_seconds'] ?? $defaults['request_timeout_seconds'])))];
}
...
function seedInputs(mixed $seeds): array {
    ...
    foreach ($seeds as $seed) {
        ...
        if (preg_match('/^https?:\/\/de\.wiki-aventurica\.de\/wiki\/(.+)$/i', $seed, $match) === 1) {
            $titles[] = title(rawurldecode((string) $match[1]));
        } elseif (preg_match('/^https?:\/\//i', $seed) !== 1) {
            $titles[] = title($seed);
        }
    }
...
function fetchHtml(string $pageTitle, int $timeout): string {
    $url = pageUrl($pageTitle);
    $headers = ['User-Agent: AvesmapsWikiDomPlayground/0.4 (https://avesmaps.de/)', 'Accept: text/html,application/xhtml+xml'];
...
function entrypointLinks(DOMXPath $xpath): array {
    $links = [];
    $queries = ['//div[contains(@class,"mw-parser-output")]//table//tr[td]//td[1]//a[@href]', '//div[contains(@class,"mw-parser-output")]//table//tr[td]//a[@href]', '//div[contains(@class,"mw-parser-output")]//ul//li//a[@href]'];
    foreach ($queries as $query) {
        foreach ($xpath->query($query) ?: [] as $node) {
            ...
            if ($target === '' || isEntrypoint($target) || !isRelevantTitle($target) || $text === '' || mb_strlen($text, 'UTF-8') < 2) {
                continue;
            }
...
function fields(DOMXPath $xpath): array {
    $fields = [];
...
function affiliationPath(array $fields, array $catchwords): array {
    foreach ($catchwords['affiliation_path_labels'] as $label) {
        ...
        $value = preg_replace('/\b(?:derographisch|geographisch|geografisch)\b\s*:?\s*[^;]+;?/iu', ' ', $raw) ?? $raw;
        $value = preg_replace('/\bpolitisch\b\s*:?\s*/iu', '', $value) ?? $value;
        $parts = preg_split('/\s*(?::|>|→|›|»|;)\s*/u', $value) ?: [];
        $path = [];
        foreach ($parts as $part) {
            $part = cleanText($part);
            if ($part === '' || preg_match('/\b(unabhaengig|unabhängig|keine|unbekannt|ungeklaert|ungeklärt)\b/iu', $part) === 1) {
                continue;
            }
            $path[] = $part;
        }
        ...
        if ($path !== []) {
            return ['raw' => $raw, 'path' => $path, 'source_field' => $label];
        }
    }
...
function temporalData(array $fields, DOMXPath $xpath, array $catchwords): array {
    ...
    foreach ($fields as $key => $value) {
        if (in_array($key, $catchwords['founded_labels'], true)) {
            $items[] = temporalItem('founded', (string) $value, 'field', $key, 0.96);
        } elseif (in_array($key, $catchwords['dissolved_labels'], true)) {
            $items[] = temporalItem('dissolved', (string) $value, 'field', $key, 0.96);
        } elseif (in_array($key, $catchwords['period_labels'], true)) {
            $items = array_merge($items, periodItems((string) $value, $key));
        }
    }
    foreach ($xpath->query('//p | //li | //tr[th and td]') ?: [] as $node) {
        $text = cleanText($node instanceof DOMNode ? ($node->textContent ?? '') : '');
        if ($text === '' || bfYears($text) === []) {
            continue;
        }
        if (containsAny($text, $catchwords['founded_context_words'])) {
            $items[] = temporalItem('founded', snippet($text, $catchwords['founded_context_words']), 'sentence', '', 0.74);
        }
        if (containsAny($text, $catchwords['dissolved_context_words'])) {
            $items[] = temporalItem('dissolved', snippet($text, $catchwords['dissolved_context_words']), 'sentence', '', 0.72);
        }
    }
    return dedupeTemporal($items);
}

function periodItems(string $text, string $field): array {
    $parts = preg_split('/\s*(?:-|–|—|bis)\s*/u', $text) ?: [];
    $items = [];
    if (isset($parts[0]) && trim($parts[0]) !== '') {
        $items[] = temporalItem('founded', (string) $parts[0], 'field', $field, 0.76);
    }
    if (isset($parts[1]) && trim($parts[1]) !== '') {
        $items[] = temporalItem('dissolved', (string) $parts[1], 'field', $field, 0.76);
    }
    return $items;
}

function temporalItem(string $kind, string $text, string $sourceType, string $field, float $confidence): array {
    $years = bfYears($text);
    return ['kind' => $kind, 'raw_text' => mb_substr(cleanText($text), 0, 500, 'UTF-8'), 'start_bf' => $years[0] ?? null, 'end_bf' => $years[1] ?? ($years[0] ?? null), 'source_type' => $sourceType, 'source_field' => $field, 'confidence' => $confidence];
}

function bfYears(string $text): array {
    $years = [];
    if (preg_match_all('/(?<!\d)(?:(v)\.?\s*)?(\d{1,4})(?:\s*[\/.]\s*\d{1,4})?\s*(?:v\.?\s*)?BF\b/iu', $text, $matches, PREG_SET_ORDER) !== false) {
        foreach ($matches as $match) {
            $value = (int) $match[2];
            $before = trim((string) ($match[1] ?? '')) !== '' || preg_match('/\d{1,4}(?:\s*[\/.]\s*\d{1,4})?\s*v\.?\s*BF\b/iu', (string) $match[0]) === 1;
            $years[] = $before ? -$value : $value;
        }
    }
...
function buildRecord(string $pageTitle, array $fields, array $pathInfo, array $temporal): array {
    ...
    $url = pageUrl($pageTitle);
    $record = ['Name' => displayName($pageTitle), 'Typ' => inferType($pageTitle, (string) ($fields['typ'] ?? $fields['art'] ?? $fields['herrschaftsgebiet'] ?? '')), 'Kontinent' => AVESMAPS_POLITICAL_DEFAULT_CONTINENT, 'Zugehoerigkeit' => (string) ($pathInfo['raw'] ?? ''), 'Zugehoerigkeit-Key' => $path !== [] ? avesmapsPoliticalSlug($path[0]) : '', 'Zugehoerigkeit-Root' => $path[0] ?? '', 'Zugehoerigkeit-Pfad' => implode(' > ', $path), 'Zugehoerigkeit-JSON' => ['path' => $path, 'source' => 'wiki-dom-playground-seed2', 'source_field' => (string) ($pathInfo['source_field'] ?? '')], 'Gruendungsdatum-Text' => (string) ($founded['raw_text'] ?? ''), 'Gruendungsdatum-StartBF' => $founded['start_bf'] ?? null, 'Gruendungsdatum-EndBF' => $founded['end_bf'] ?? null, 'Gruendungsdatum-JSON' => $founded ?: [], 'Aufgeloest-Text' => (string) ($dissolved['raw_text'] ?? ''), 'Aufgeloest-StartBF' => $dissolved['start_bf'] ?? null, 'Aufgeloest-EndBF' => $dissolved['end_bf'] ?? null, 'Aufgeloest-JSON' => $dissolved ?: [], 'Wiki-Link' => $url, 'raw_json' => ['source' => 'wiki-dom-playground-seed2', 'fields' => $fields, 'path' => $pathInfo, 'temporal' => $temporal]];
    return avesmapsPoliticalNormalizeWikiRecord($record);
}
...
function enqueue(array &$queue, array &$queued, string $pageTitle, string $source): void {
    $pageTitle = title($pageTitle);
    $key = stableKey($pageTitle);
    if ($key === '' || isset($queued[$key])) {
        return;
    }
    $queued[$key] = true;
    $queue[] = ['title' => $pageTitle, 'source' => $source];
}
...
function isEntrypoint(string $pageTitle): bool {
    return preg_match('/\/Liste$/u', title($pageTitle)) === 1;
}

function titleFromHref(string $href): string {
    return str_starts_with(trim($href), '/wiki/') ? title(rawurldecode(substr(trim($href), 6))) : '';
}

function isRelevantTitle(string $pageTitle): bool {
    return $pageTitle !== '' && !str_contains($pageTitle, '#') && preg_match('/^(Datei|File|Kategorie|Category|Spezial|Special|Hilfe|Help|Vorlage|Template|Benutzer|Diskussion):/iu', $pageTitle) !== 1;
}
...
function dedupeTemporal(array $items): array {
    $out = [];
    foreach ($items as $item) {
        if (is_array($item)) {
            $out[implode('|', [(string) ($item['kind'] ?? ''), (string) ($item['start_bf'] ?? ''), (string) ($item['end_bf'] ?? ''), (string) ($item['raw_text'] ?? '')])] = $item;
        }
    }
    return array_values($out);
}

function containsAny(string $text, array $words): bool {
    $key = labelKey($text);
    foreach ($words as $word) {
        if ($word !== '' && str_contains($key, (string) $word)) {
            return true;
        }
    }
    return false;
...
function snippet(string $text, array $words): string {
    foreach ($words as $word) {
        if ($word !== '' && preg_match('/.{0,90}' . preg_quote((string) $word, '/') . '.{0,160}/iu', $text, $match) === 1) {
            return cleanText((string) $match[0]);
        }
    }
    return mb_substr(cleanText($text), 0, 260, 'UTF-8');
}

function inferType(string $pageTitle, string $fallback): string {
    if (trim($fallback) !== '') {
        return trim($fallback);
    }
    foreach (['Kaiserreich', 'Königreich', 'Bergkönigreich', 'Imperium', 'Republik', 'Herzogtum', 'Fürstentum', 'Grafschaft', 'Baronie', 'Freiherrschaft', 'Markgrafschaft', 'Pfalzgrafschaft', 'Jarltum', 'Sultanat', 'Emirat', 'Theokratie', 'Komturei', 'Domäne', 'Ordensland', 'Reich', 'Staat'] as $type) {
        if (preg_match('/\b' . preg_quote($type, '/') . '\b/iu', $pageTitle) === 1) {
            return $type;
        }
    }
    return '';
...
function displayName(string $pageTitle): string {
    return trim((string) preg_replace('/\s+\((?:Staat|Reich|Historisch|Region)\)\s*$/iu', '', str_replace('_', ' ', title($pageTitle))));
}

function title(string $pageTitle): string {
    $pageTitle = str_replace('_', ' ', rawurldecode(trim($pageTitle)));
    $pageTitle = preg_replace('/#.*$/u', '', $pageTitle) ?? $pageTitle;
    return trim($pageTitle);
}

function pageUrl(string $pageTitle): string {
    return WIKI_DOM_BASE_URL . str_replace('%2F', '/', rawurlencode(str_replace(' ', '_', title($pageTitle))));
}

function stableKey(string $value): string {
    return avesmapsPoliticalSlug(title($value));
}
...
function ensureTestTable(PDO $pdo): void {
    if ($pdo->query("SHOW TABLES LIKE 'political_territory_wiki'")->fetchColumn() === false) {
        throw new RuntimeException('Basis-Tabelle political_territory_wiki fehlt.');
    }
