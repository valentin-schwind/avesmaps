...
(function () {
	...
	function dialogHtml() {
		return (
			...
			'<div class="tsi-rule tsi-rule--wide">' + iconImg(pathIcon("Gebirgspass")) + "<div>" + tr("transport.speedInfo.slopeRule", "<b>Steigung.</b> Entscheidend ist die <em>Steilheit in Prozent</em>, nicht der Höhenunterschied. Gerechnet wird nach dem <b>Leistungskilometer</b> (DIN 33466), der Marschzeitrechnung der Alpenvereine: eine Meile flaches Land ist eine Leistungsmeile, je 100 Schritt Aufstieg kommt eine dazu, und je 150 Schritt Abstieg — aber nur an Hängen über 20 % Gefälle. Damit dauert eine Etappe bei <b>5 % Steigung die Hälfte länger, bei 10 % doppelt so lang, bei 20 % dreifach</b>; ab 30 % ist bei vierfacher Zeit Schluss. <b>Bergab kostet nichts, solange es flacher als 20 % ist — und schneller als die Ebene wird es nie.</b> Über den Koschberge-Pass (24 %) wird aus einer Reichsstraße so rund 1,3 statt 4,5 Meilen/h. Auf Flüssen und Meeren zählt kein Gelände — dort entscheidet allein die Strömung. Wo keine Höhen erfasst sind, zählt allein der Wegtyp; die Etappen-Infobox zeigt dann auch keine Zeile „Auf und ab“. ⚠️ Die 20 %-Schwelle gilt je Teilstück, nicht im Etappenmittel: echtes Gelände ist nicht glatt, deshalb kann eine Etappe etwas langsamer sein als die Faustregel für ihr Durchschnittsgefälle vermuten lässt.") + "</div></div>" +
			"</div></div></div>"
		);
	}
