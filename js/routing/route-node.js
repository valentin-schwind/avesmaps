...
function shouldShowRoutePathDisplayName(segment) {
	...
	if (displayName === subtype) {
		return false;  // bare subtype ("Flussweg") is no name -- "über Flussweg" is noise
	}
	const autogeneratedPattern = new RegExp(`^${escapeRouteDisplayRegex(subtype)}-\\d+$`);
	if (autogeneratedPattern.test(displayName)) {
		return false;
	}
	// Generic technical node names -- a single token + "-<number>" ("Meer-835", "Kreuzung-549",
	// "Seeweg-12") are auto-generated, never a real Wiki way name. Without this, extending way names
	// to sea routes surfaced the whole "Meer-835, Meer-836, ..." chain (the prefix "Meer" differs
	// from the subtype "Seeweg", so the subtype-specific pattern above missed it).
	return !/^\S+-\d+$/.test(displayName);
}
