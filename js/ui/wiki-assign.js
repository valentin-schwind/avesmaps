...
function avesmapsWikiAssignMarkup(modell, skin) {
	...
	const hinweisMarkup = avesmapsWikiAssignText(modell.hinweis) === ""
		? ""
		: "<div" + avesmapsWikiAssignKlasse(skin.hinweis) + ">" + avesmapsWikiAssignEsc(modell.hinweis) + "</div>";
	...
	if (modell.suchfeld) {
		...
		teile.push("<" + skin.listeTag + avesmapsWikiAssignKlasse(skin.listeKlasse) + ">"
			...
			+ avesmapsWikiAssignEsc(modell.suchfeld.label) + "</" + skin.nameTag + ">"
		...
		teile.push('<div class="' + avesmapsWikiAssignEsc(skin.trefferListe) + '" role="listbox">'
			+ (modell.treffer.length === 0
				? "<div" + avesmapsWikiAssignKlasse(skin.trefferLeer) + ">" + avesmapsWikiAssignEsc(modell.trefferLeerText) + "</div>"
				: modell.treffer.map((treffer) => avesmapsWikiAssignTrefferMarkup(skin, treffer)).join(""))
			+ "</div>");
...
function avesmapsWikiAssignMount(behaelter, optionen) {
	const opt = optionen || {};
	const erklaerung = (typeof avesmapsWikiAssignSubject === "function")
		? avesmapsWikiAssignSubject(opt.subject)
		: null;
	const skin = avesmapsWikiAssignSkin(opt.skin);
	if (!behaelter || !erklaerung || !skin) {
		// 💣 Kein stiller Leerlauf: eine unbekannte Objektart oder Huelle ist ein Bau-, kein
		// Datenfehler, und ein leerer Kasten sieht aus wie „hier gibt es nichts“.
		if (behaelter) {
			behaelter.textContent = !erklaerung
				? 'Wiki-Zuweisung: keine Erklärung für „' + String(opt.subject) + '“.'
				: 'Wiki-Zuweisung: unbekannte Hülle „' + String(opt.skin) + '“.';
		}
		return { lies: () => ({ name: "", wiki_url: "", wiki_key: "", kein_artikel: false }), neuLaden: () => {}, zerstoeren: () => {} };
	}

	let daten = { artikel: null, keinArtikel: false, kartenwerte: {}, handgesetzt: [], gesperrt: {}, listen: {} };
	let ui = { modus: "offen", suchtext: "", treffer: [], aktiv: 0, syncZeilen: [] };
	let tippUhr = null;
	let laufendeSuche = 0;
	...
	function zeichne() {
		...
		if (feld) {
			feld.focus();
			// Der Zeiger ans Ende, sonst springt er bei jedem Neuzeichnen an den Anfang.
	...
	function neuLaden() {
		...
		return Promise.resolve(roh).then((wert) => {
			...
			ui = { modus: daten.artikel ? "zugewiesen" : "offen", suchtext: "", treffer: [], aktiv: 0, syncZeilen: [] };
			zeichne();
		}, () => {
			behaelter.textContent = "Wiki-Zuweisung: der Stand konnte nicht gelesen werden.";
		});
	...
	function trefferWaehlen(index) {
		...
		Promise.resolve(typeof opt.zuweisen === "function" ? opt.zuweisen(treffer) : null).then(() => {
			...
			ui = { modus: "zugewiesen", suchtext: "", treffer: [], aktiv: 0, syncZeilen: [] };
			zeichne();
		});
	}
	...
	function aufAktion(aktion) {
		if (aktion === "zuweisen" || aktion === "aendern") {
			ui = { modus: "suche", suchtext: "", treffer: [], aktiv: 0, syncZeilen: [] };
			zeichne();
			sucheAnstossen(true);
			return;
		}
		...
		if (aktion === "entfernen") {
			Promise.resolve(typeof opt.loesen === "function" ? opt.loesen() : null).then(() => {
				daten.artikel = null;
				ui = { modus: "offen", suchtext: "", treffer: [], aktiv: 0, syncZeilen: [] };
				zeichne();
			});
			return;
		}
