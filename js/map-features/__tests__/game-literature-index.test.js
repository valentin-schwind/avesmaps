...
assert.strictEqual(avesmapsGameLiteratureToRenderShape({ public_id: "y", bf_year: 1030 }).yearLabel, "1030 BF");
...
const linked = avesmapsGameLiteratureToRenderShape({
	public_id: "z", title: "T",
...
assert.deepStrictEqual(avesmapsGameLiteratureToRenderShape({ public_id: "y" }).links, []);
...
const index = avesmapsBuildGameLiteratureIndex(catalog);

// S1 (by public_id): advA BEGINS here; advB PLAYS here (start elsewhere) -> spoiler set.
const beginsAtS1 = avesmapsSelectGameLiteratureEntries(index, { publicId: "S1" }, "start");
assert.strictEqual(beginsAtS1.length, 1);
assert.strictEqual(beginsAtS1[0].adv.public_id, "advA");
const playsAtS1 = avesmapsSelectGameLiteratureEntries(index, { publicId: "S1" }, "play");
assert.strictEqual(playsAtS1.length, 1);
assert.strictEqual(playsAtS1[0].adv.public_id, "advB");
assert.strictEqual(avesmapsSelectGameLiteratureEntries(index, { publicId: "S1" }, "all").length, 2);
...
const ferdokKey = avesmapsNormalizeGameLiteratureKey("wiki:ferdok");
assert.strictEqual(ferdokKey, "ferdok"); // 'wiki:' stripped, hyphens/prefix normalized away
const byKey = avesmapsSelectGameLiteratureEntries(index, { key: ferdokKey }, "play");
...
const both = avesmapsSelectGameLiteratureEntries(index, { publicId: "S1", key: avesmapsNormalizeGameLiteratureKey("wiki:gareth") }, "all");
...
const terrIndex = avesmapsBuildGameLiteratureIndex([
...
const terrHit = avesmapsSelectGameLiteratureEntries(terrIndex, { key: avesmapsNormalizeGameLiteratureKey("wiki:koenigreich-garetien") }, "start");
...
assert.deepStrictEqual(avesmapsSelectGameLiteratureEntries(index, {}, "start"), []);
...
const ti = avesmapsBuildGameLiteratureIndex(territoryCatalog);
const mrKey = avesmapsNormalizeGameLiteratureKey("wiki:mittelreich");
const gKey = avesmapsNormalizeGameLiteratureKey("wiki:grafschaft-g");
const aKey = avesmapsNormalizeGameLiteratureKey("wiki:baronie-a");

// Root (Mittelreich): both adventures begin in its subtree; advX also PLAYS there (play place is a MR barony).
assert.strictEqual(avesmapsSelectGameLiteratureEntries(ti, { territoryKey: mrKey }, "start").length, 2);
const playMR = avesmapsSelectGameLiteratureEntries(ti, { territoryKey: mrKey }, "play");
...
assert.strictEqual(avesmapsSelectGameLiteratureEntries(ti, { territoryKey: gKey }, "start").length, 2);
// Leaf (Baronie A): only advX begins there.
const beginA = avesmapsSelectGameLiteratureEntries(ti, { territoryKey: aKey }, "start");
...
assert.strictEqual(avesmapsSelectGameLiteratureEntries(ti, { territoryKey: mrKey }, "all").length, 2);
// A territory outside any path -> nothing.
assert.strictEqual(avesmapsSelectGameLiteratureEntries(ti, { territoryKey: avesmapsNormalizeGameLiteratureKey("wiki:andergast") }, "all").length, 0);
...
const rp = avesmapsBuildGameLiteratureIndex(regionPathCatalog);
// Region: the exact public_id (what the resolver stores = the label's public_id) AND the wiki-key fallback hit.
assert.strictEqual(avesmapsSelectGameLiteratureEntries(rp, { regionPublicId: "R1" }, "start").length, 1);
assert.strictEqual(avesmapsSelectGameLiteratureEntries(rp, { regionKey: avesmapsNormalizeGameLiteratureKey("wiki:raschtulswall") }, "start").length, 1);
assert.strictEqual(avesmapsSelectGameLiteratureEntries(rp, { regionPublicId: "R1" }, "play").length, 0); // it BEGINS here, not plays
// Dedup across region public_id + key for the same adventure -> once.
assert.strictEqual(avesmapsSelectGameLiteratureEntries(rp, { regionPublicId: "R1", regionKey: avesmapsNormalizeGameLiteratureKey("wiki:raschtulswall") }, "all").length, 1);
...
const bornKey = avesmapsNormalizeGameLiteratureKey("bornstrasse");
assert.strictEqual(avesmapsSelectGameLiteratureEntries(rp, { pathKey: bornKey }, "play").length, 1);
assert.strictEqual(avesmapsSelectGameLiteratureEntries(rp, { pathPublicId: "P1seg" }, "play").length, 1);
assert.strictEqual(avesmapsSelectGameLiteratureEntries(rp, { pathKey: bornKey }, "start").length, 0); // it PLAYS here, not begins
console.log("path index ok");

// ---- edition sort key ("nach Edition": DSA5 > DSA4.1 > DSA4 > … > DSA1 > non-DSA > empty; title tiebreak) ----
assert.strictEqual(avesmapsGameLiteratureEditionSortKey("DSA1"), -1);
assert.strictEqual(avesmapsGameLiteratureEditionSortKey("DSA5"), -5);
assert.strictEqual(avesmapsGameLiteratureEditionSortKey("DSA3.5"), -3.5);
assert.strictEqual(avesmapsGameLiteratureEditionSortKey("DSA4.1 Basis"), -4.1);
assert.strictEqual(avesmapsGameLiteratureEditionSortKey("DSA1-Ausbau"), -1);
assert.strictEqual(avesmapsGameLiteratureEditionSortKey("DSA4 / DSA5"), -4); // first edition wins
assert.strictEqual(avesmapsGameLiteratureEditionSortKey("Aventuria 2.0"), 1000); // non-DSA ruleset
assert.strictEqual(avesmapsGameLiteratureEditionSortKey(""), 1001); // no edition sorts last
const edSorted = ["DSA5", "", "DSA1", "DSA4.1", "Aventuria 2.0", "DSA4"]
	.sort((a, b) => avesmapsGameLiteratureEditionSortKey(a) - avesmapsGameLiteratureEditionSortKey(b) || a.localeCompare(b, "de"));
